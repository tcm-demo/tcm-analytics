...
var INV_GOODS_KEY = 'tcm_inv_goods_v5';
...
var INV_GOODS_SEED = [
  { goodsId: 'g-01', name: '娃娃菜', code: '6901234500017', cat: '叶菜类', catPath: '生鲜/蔬菜/叶菜/娃娃菜', grade: '', unit: 'kg', spec: '500g/份', sale: 1, expiryManaged: false, warehouse: 100, shelf: 26, avgCost: 2.8, supplier: '青浦绿蔬合作社', storeId: 'S2001', batches: [{ no: 'RK20260908001', pdate: '2026-09-08', shelfLife: 5, shelfLifeType: 1, exp: '2026-09-13', w: 70, s: 18 }, { no: 'RK20260901001', pdate: '2026-09-01', shelfLife: 5, shelfLifeType: 1, exp: '2026-09-06', w: 30, s: 8 }] },
  { goodsId: 'g-02', name: '上海青', code: '6901234500024', cat: '叶菜类', catPath: '生鲜/蔬菜/叶菜/上海青', grade: '', unit: 'kg', spec: '400g/份', sale: 1, expiryManaged: false, warehouse: 88, shelf: 24, avgCost: 2.2, supplier: '青浦绿蔬合作社', storeId: 'S2001', batches: [{ no: 'RK20260901001', pdate: '2026-09-01', shelfLife: 4, shelfLifeType: 1, exp: '2026-09-05', w: 60, s: 24 }, { no: 'RK20260911001', pdate: '2026-09-11', shelfLife: 4, shelfLifeType: 1, exp: '2026-09-15', w: 28, s: 0 }] },
  ...
  { goodsId: 'g-04', name: '土豆', code: '6901234500048', cat: '根茎类', catPath: '生鲜/蔬菜/根茎/土豆', grade: '', unit: 'kg', spec: '称重', sale: 1, expiryManaged: false, warehouse: 200, shelf: 60, avgCost: 3.4, supplier: '崧泽基地直供', storeId: 'S2001', batches: [{ no: 'RK20260901002', pdate: '2026-09-01', shelfLife: 30, shelfLifeType: 1, exp: '2026-10-01', w: 120, s: 60, cost: 3.2 }, { no: 'RK20260908002', pdate: '2026-09-08', shelfLife: 30, shelfLifeType: 1, exp: '2026-10-08', w: 80, s: 0, cost: 3.8 }] },
  ...
  { goodsId: 'g-11', name: '光明鲜牛奶', code: '6901234500116', cat: '乳品烘焙', catPath: '乳品/乳制品/牛奶/鲜牛奶', grade: '', unit: '盒', spec: '950ml/盒', sale: 1, expiryManaged: true, warehouse: 30, shelf: 24, avgCost: 11.9, supplier: '光明乳业', storeId: 'S2001', batches: [{ no: 'RK20260912005', pdate: '2026-09-12', shelfLife: 7, shelfLifeType: 1, exp: '2026-09-19', w: 16, s: 10 }, { no: 'RK20260912005-2', pdate: '2026-09-09', shelfLife: 7, shelfLifeType: 1, exp: '2026-09-16', w: 14, s: 14 }] },
      ...
      '<div style="flex:1;border:1px solid #e9eef7;border-radius:6px;padding:12px;background:#fafbfd">' +
        ...
        '<select class="ic-search" style="width:100%;margin-bottom:6px" id="ilBoxBatch" onchange="invListBoxBatchInfo()">' +
          (r.batches.length ? r.batches.map(function (b) {
            ...
            var rmTxt = rm === null ? '无有效期' : (!r.expiryManaged ? '不预警' : (rm < 0 ? '已过期' + Math.abs(rm) + '天' : '剩' + rm + '天'));
...
function invListBoxBatchInfo() {
  ...
  else if (!g.expiryManaged) rmHtml = '<span style="color:#8a93a3">不预警</span>';
...
// 剩余有效期统一徽标：非效期管理商品（g.expiryManaged 为假）一律灰色「不预警」，不参与红/橙/绿预警色
function msInvRemainBadge(g, remain) {
  if (remain == null || remain === undefined) return '<span style="color:#8a93a3">—</span>';
  if (g && !g.expiryManaged) return '<span style="color:#8a93a3">不预警</span>';
  if (remain <= 0) return '<span style="color:#ec2d30;font-weight:600">已过期 ' + Math.abs(remain) + ' 天</span>';
  if (remain <= 7) return '<span style="color:#f57316">剩 ' + remain + ' 天</span>';
  return '<span style="color:#3eb27e">剩 ' + remain + ' 天</span>';
}
...
function invListOpenBatch(goodsId) {
  ...
  var rows = b.length ? b.map(function (x, i) {
    ...
    var remainHtml = msInvRemainBadge(g, remain);
    return '<tr><td style="text-align:center;vertical-align:middle;padding:8px 6px;color:#999">' + (i + 1) + '</td><td style="text-align:left;vertical-align:middle;padding:8px 10px">' + msInvEsc(x.no) + '</td>' +
      ...
      '<td style="text-align:left;vertical-align:middle;padding:6px 10px"><input type="date" id="ibx_pd_' + i + '" value="' + (x.pdate || '') + '" style="width:100%;max-width:108px;font-size:11px;height:26px;border:1px solid #dfe3ed;border-radius:3px;padding:0 4px;color:#0b1019" title="入库填错可在此修正；仅改生产日期时到期日按保质期自动重算，点「保存修改」生效"></td>' +
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px">' + msInvLifeText(x) + '</td>' +
      '<td style="text-align:left;vertical-align:middle;padding:6px 10px"><input type="date" id="ibx_ex_' + i + '" value="' + (x.exp || '') + '" style="width:100%;max-width:108px;font-size:11px;height:26px;border:1px solid #dfe3ed;border-radius:3px;padding:0 4px;color:#0b1019" title="入库填错可在此修正；修改后保质期自动反算，点「保存修改」生效"></td>' +
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px">' + remainHtml + '</td></tr>';
  ...
  var body = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">' +
    '<div style="font-size:13px;font-weight:600;color:#0b1019">' + msInvEsc(g.name) + '</div>' +
    '<button class="ic-btn" onclick="invListToggleExp(\'' + g.goodsId + '\')">' + (g.expiryManaged ? '效期管理：已开启' : '效期管理：已关闭') + '</button></div>' +
    '<div style="font-size:12px;color:#8a93a3;margin-bottom:6px">' + msInvEsc(g.code) + ' · ' + msInvEsc(g.spec) + '/' + g.unit + ' · 共 ' + b.length + ' 个批次（FIFO 先产先出）' +
    (g.expiryManaged ? '' : '<span style="color:#8a93a3"> · 未开启效期管理，效期仅作参考、不预警，盘点按总数</span>') + '</div>' +
    '<div style="font-size:11px;color:#8a93a3;margin-bottom:10px">生产日期 / 有效期至可直接修改（入库填错后的修正入口）：改生产日期按保质期自动重算到期日，改到期日自动反算保质期；已产生的入库/盘点/退货单据为历史快照，不随之回写。</div>' +
    ...
    '</table></div>';
  window._invBatchGid = goodsId;
  msInvModal({ title: '商品批次库存', width: 'min(860px,94vw)', body: body, onOk: 'invListBatchSave()', okText: '保存修改', cancelText: '关闭' });
}

// B4：效期管理开关（库存主数据唯一口径入口）——在批次弹窗切换，持久化后重开刷新
function invListToggleExp(goodsId) {
  var g = null; INV_GOODS.forEach(function (x) { if (x.goodsId === goodsId) g = x; });
  if (!g) return;
  g.expiryManaged = !g.expiryManaged;
  invListPersist();
  msInvToast(g.name + ' 效期管理已' + (g.expiryManaged ? '开启：后续新开盘点按批次展开' : '关闭：效期仅作参考、盘点按总数'));
  invListRender();
  invListOpenBatch(goodsId);
}

// 批次效期修正保存：输入框只暂存，点「保存修改」统一校验+提交（主数据唯一口径）
// 口径：仅改生产日期 → 到期日按保质期自动重算；改到期日 → 保质期自动反算；两者都改 → 按填写值；历史单据快照不回写（留痕）
function invListBatchSave() {
  var goodsId = window._invBatchGid;
  var g = null; INV_GOODS.forEach(function (x) { if (x.goodsId === goodsId) g = x; });
  if (!g) { msInvCloseModal(); return; }
  var bs = g.batches || [], plans = [], errs = [];
  for (var i = 0; i < bs.length; i++) {
    var b = bs[i];
    var pdEl = document.getElementById('ibx_pd_' + i), exEl = document.getElementById('ibx_ex_' + i);
    if (!pdEl && !exEl) continue;
    var npd = pdEl ? pdEl.value : b.pdate, nex = exEl ? exEl.value : b.exp;
    if (!npd || !nex) { errs.push('批次 ' + b.no + '：生产日期与到期日均不能为空'); continue; }
    if (msInvPdateVal(nex) <= msInvPdateVal(npd)) { errs.push('批次 ' + b.no + '：到期日必须晚于生产日期'); continue; }
    var pdChanged = npd !== b.pdate, exChanged = nex !== b.exp;
    if (pdChanged || exChanged) plans.push({ b: b, npd: npd, nex: nex, pdChanged: pdChanged, exChanged: exChanged });
  }
  if (errs.length) { msInvToast(errs[0] + (errs.length > 1 ? '（共 ' + errs.length + ' 处需修正）' : '')); return; }
  if (!plans.length) { msInvCloseModal(); return; }
  var msgs = [];
  plans.forEach(function (p) {
    if (p.pdChanged && !p.exChanged && p.b.shelfLife) { p.nex = invCkAddDays(p.npd, p.b.shelfLife); msgs.push(p.b.no + ' 到期日按保质期 ' + p.b.shelfLife + ' 天重算为 ' + p.nex); }
    else if (p.exChanged && !p.pdChanged) { p.b.shelfLife = Math.round((msInvPdateVal(p.nex) - msInvPdateVal(p.npd)) / 86400000); p.b.shelfLifeType = 1; msgs.push(p.b.no + ' 保质期同步为 ' + p.b.shelfLife + ' 天'); }
    p.b.pdate = p.npd; p.b.exp = p.nex;
  });
  invListPersist();
  invListRender();
  invListOpenBatch(goodsId);
  msInvToast('已保存 ' + plans.length + ' 个批次的效期修正' + (msgs.length ? '：' + msgs.join('；') : ''));
}
...
var INV_ENTRY_KEY = 'tcm_inv_entry_v3';
...
function invEntrySeed() {
  return [
    ...
    { no: 'RK20260912005', date: '2026-09-12', storeId: 'S2001', supplier: '光明乳业', items: [{ name: '光明鲜牛奶', code: '6901234500116', spec: '950ml/盒', batch: 'RK20260912005', pdate: '2026-09-12', shelfLife: 7, shelfLifeType: 1, qty: 16, price: 11.9 }, { name: '光明鲜牛奶', code: '6901234500116', spec: '950ml/盒', batch: 'RK20260912005-2', pdate: '2026-09-09', shelfLife: 7, shelfLifeType: 1, qty: 14, price: 11.9 }], cost: 357.0, status: 20 }
  ];
}
var INV_ENTRY = [];
...
function invEntryAddStock(r) {
  r.items.forEach(function (it) {
    ...
    // E1 修复：同单同商品多生产日期按「单号+生产日期/到期日」维批——首个日期组批次号=入库单号，后续组加 -2/-3 后缀，杜绝吞日期
    var pd = it.pdate || '', ex = it.exp || '';
    var exist = null, cnt = 0;
    for (var i = 0; i < g.batches.length; i++) {
      var bb = g.batches[i];
      if (bb.no === r.no || bb.no.indexOf(r.no + '-') === 0) {
        cnt++;
        if ((bb.pdate || '') === pd && (bb.exp || '') === ex) exist = bb;
      }
    }
    var bno;
    if (exist) { exist.w += it.qty; bno = exist.no; }
    else {
      bno = cnt === 0 ? r.no : r.no + '-' + (cnt + 1);
      g.batches.unshift({ no: bno, pdate: it.pdate || '', shelfLife: it.shelfLife, shelfLifeType: it.shelfLifeType || 1, exp: it.exp || '', w: it.qty, s: 0 });
    }
    invFlowAdd(10, it.code, it.name, it.qty, '仓库', r.no, bno);
  });
  invListPersist();
}
...
function invTfDo(no, act) {
  // ⚠️ 已知待处理（规划标记，2026-09-13）：调拨未批次化——发货只减总数不扣批次、完成只加总数不建批，
  // 效期商品调拨后「批次合计 ≠ 商品总数」。短期不用调拨，暂缓处理；后续启用调拨前必须批次化（对齐退货按批核销口径）。
...
var INV_RT_KEY = 'tcm_inv_return_v4';
// 该商品对应已完成入库单的批次候选（来源：已完成入库单明细；E1 拆批后同单同商品多日期=多批，每批一行）
function invRtCandidates(code, supplier) {
  ...
  for (var i = 0; i < INV_ENTRY.length; i++) {
    ...
    for (var j = 0; j < r.items.length; j++) {
      ...
      if (!g) continue;
      var matched = [];
      for (var k = 0; k < g.batches.length; k++) {
        var b = g.batches[k];
        if (b.no === r.no || b.no.indexOf(r.no + '-') === 0) matched.push(b);
      }
      if (!matched.length) matched = [{ no: r.no, pdate: it.pdate || '', shelfLife: it.shelfLife, shelfLifeType: it.shelfLifeType || 1, exp: it.exp || '', w: 0, s: 0 }];
      matched.forEach(function (b) {
        var src = it;
        if (matched.length > 1) {
          for (var m = 0; m < r.items.length; m++) {
            var x = r.items[m];
            if (x.code === code && (x.pdate || '') === (b.pdate || '')) { src = x; break; }
          }
        }
        // 退货只从仓库出库：可退库存按仓库维度取 batch.w，与提交扣减口径一致
        out.push({ rkNo: r.no, batch: b.no, rkQty: src.qty, cost: src.price, cur: b.w, pdate: b.pdate || '', shelfLife: b.shelfLife != null ? b.shelfLife : src.shelfLife, shelfLifeType: b.shelfLifeType || 1, exp: b.exp || '' });
      });
    }
  }
  return out;
...
function invRtSeed() {
  return [
    ...
    { no: 'TH20260912001', date: '2026-09-12', storeId: 'S2001', supplier: '光明乳业', items: [{ name: '光明鲜牛奶', code: '6901234500116', spec: '950ml/盒', rkNo: 'RK20260912005', batch: 'RK20260912005-2', pdate: '2026-09-09', shelfLife: 7, shelfLifeType: 1, exp: '2026-09-16', rkQty: 14, cur: 14, qty: 2, cost: 11.9, price: 11.9 }], amount: 23.8, reason: '临期退回', status: 10 }
  ];
}
var INV_RT = [];
...
function invRtInit() {
  ...
  el.innerHTML =
    ...
    '</div>' +
    '<div style="flex:1;min-height:0;margin:10px 10px 4px;background:#fff;border-radius:4px;display:flex;flex-direction:column;border:1px solid #e9eef7;overflow:hidden">' +
      '<div class="table-wrap" style="flex:1;overflow:auto;min-height:0"><table style="width:100%;min-width:1000px">' +
        '<thead><tr><th style="width:50px">序号</th><th style="width:170px">退货单号</th><th style="width:100px">退货日期</th><th style="width:130px">门店</th><th style="width:140px">供应商</th><th style="width:70px">商品数</th><th style="width:110px">退货金额(元)</th><th style="width:100px">退货原因</th><th style="width:80px">状态</th><th style="width:150px">操作</th></tr></thead>' +
...
function invRtEditRender() {
  var storeLabel = msInvScopeStoreLabel();
  // 列结构对齐 Vue Returnedgoods：序号/名称/编码/规格按商品合并（rowspan），入库单号/批次号/效期每批一行
  var rowsHtml = '';
  if (INV_RT_EDIT_ITEMS.length) {
    var gi = 0, gIdx = 0;
    while (gi < INV_RT_EDIT_ITEMS.length) {
      var gCode = INV_RT_EDIT_ITEMS[gi].code, gN = 0;
      for (var gj = gi; gj < INV_RT_EDIT_ITEMS.length && INV_RT_EDIT_ITEMS[gj].code === gCode; gj++) gN++;
      var headTds =
        '<td rowspan="' + gN + '" style="text-align:center;color:#999">' + (gIdx + 1) + '</td>' +
        '<td rowspan="' + gN + '" style="font-weight:600;color:#0b1019">' + msInvEsc(INV_RT_EDIT_ITEMS[gi].name) + '</td>' +
        '<td rowspan="' + gN + '" style="font-size:12px">' + msInvEsc(gCode) + '</td>' +
        '<td rowspan="' + gN + '" style="font-size:12px">' + msInvEsc(INV_RT_EDIT_ITEMS[gi].spec || '') + '</td>';
      for (var k = gi; k < gi + gN; k++) {
        var it = INV_RT_EDIT_ITEMS[k];
        var maxQ = Math.min(it.rkQty, it.cur);
        var remain = it.exp ? msInvRemainDays(it.exp) : null;
        var remHtml = remain == null ? '<span style="color:#8a93a3">—</span>' : (remain <= 0 ? '<span style="color:#ec2d30;font-weight:600">已过期 ' + Math.abs(remain) + ' 天</span>' : (remain <= 7 ? '<span style="color:#f57316">剩 ' + remain + ' 天</span>' : '<span style="color:#3eb27e">剩 ' + remain + ' 天</span>'));
        rowsHtml += '<tr>' + headTds +
          '<td style="font-size:11px;white-space:nowrap">' + msInvEsc(it.rkNo) + '</td>' +
          '<td style="font-size:11px;white-space:nowrap">' + msInvEsc(it.batch || it.rkNo) + '</td>' +
          '<td style="text-align:right">' + it.rkQty + '</td>' +
          '<td style="text-align:right">' + it.cur + '</td>' +
          '<td><input id="rtq_' + k + '" type="number" min="1" class="ic-search" style="width:76px" value="' + it.qty + '" onblur="invRtQtyBlur(' + k + ')" oninput="invRtRecalc()"></td>' +
          '<td style="text-align:right">' + it.cost.toFixed(2) + '</td>' +
          '<td><input id="rtp_' + k + '" type="number" min="0" step="0.01" class="ic-search" style="width:80px" value="' + it.price + '" onblur="invRtPriceBlur(' + k + ')" oninput="invRtRecalc()"></td>' +
          '<td style="text-align:right" id="rtsub_' + k + '">¥' + (it.qty * it.price).toFixed(2) + '</td>' +
          '<td style="font-size:12px;white-space:nowrap">' + (it.pdate || '—') + '</td>' +
          '<td style="font-size:12px;white-space:nowrap">' + msInvLifeText(it) + '</td>' +
          '<td style="font-size:12px;white-space:nowrap">' + remHtml + '</td>' +
          '<td><button class="ic-btn" style="color:#fc4b52" onclick="invRtRemoveItem(' + k + ')">移除</button></td></tr>';
        headTds = '';
      }
      gIdx++; gi += gN;
    }
  } else {
    rowsHtml = '<tr><td colspan="16" style="text-align:center;color:#999;padding:24px">尚未添加退货商品，点击「按入库单选源」从已完成入库单的批次中选择</td></tr>';
  }
  var body =
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px 24px;margin-bottom:12px">' +
    ...
    '</div>' +
    '<div style="font-size:12px;font-weight:600;color:#1a2233;margin-bottom:6px">退货商品明细 <span style="color:#8a93a3;font-weight:400">（每批一行，同单多生产日期已拆批；最大可退=min(该批入库量, 仓库库存)；退货价≤进货价）</span></div>' +
    '<style>.icm-tb th{padding:9px 12px !important}.icm-tb td{padding:9px 12px !important}</style>' +
    '<div class="icm-tb" style="max-height:420px;overflow:auto;border:1px solid #e9eef7;border-radius:4px"><table style="width:100%;min-width:1400px">' +
      '<thead><tr><th style="width:46px">序号</th><th>商品名称</th><th style="width:110px">编码/条码</th><th style="width:90px">商品规格</th><th style="width:140px">入库单号</th><th style="width:130px">批次号</th><th style="width:70px">进货数量</th><th style="width:70px">库存数量</th><th style="width:84px">退货数量</th><th style="width:76px">进货价</th><th style="width:88px">退货价(≤进价)</th><th style="width:90px">退货金额</th><th style="width:92px">生产日期</th><th style="width:80px">保质期</th><th style="width:96px">剩余有效期</th><th style="width:60px">操作</th></tr></thead>' +
      '<tbody>' + rowsHtml + '</tbody>' +
      '<tfoot><tr style="background:#f7f9fc"><td colspan="11" style="text-align:right;font-weight:600">合计</td><td style="text-align:right;font-weight:600;color:#d4380d" id="rtTotal">¥0.00</td><td colspan="4"></td></tr></tfoot>' +
    '</table></div>' +
    '<div style="margin-top:10px"><button class="ic-btn ic-btn-pri" onclick="invRtPickGoods(\'batch\')">按入库单选源</button><span style="font-size:11px;color:#8a93a3;margin-left:8px">支持「按批次」勾选或「先进先出」自动分摊两种方式</span></div>';
  msInvModal({ title: (window._rtStatus === '20' ? '查看退货单(已完成·只读)' : (window._rtNo && (function () { var f = false; INV_RT.forEach(function (x) { if (x.no === window._rtNo) f = true; }); return f; })() ? '编辑退货单(待提交)' : '新增退货单')), width: 'min(1500px,98vw)', body: body, footer: false });
...
function invRtPickGoods(mode) {
  mode = mode || 'batch';
  window._rtCandMode = mode;
  var sup = window._rtSup || '青浦绿蔬合作社';
  // 从已完成入库单按供应商汇总候选行（E1 拆批后：入库单×商品×批次=一行，每批带自己的效期）
  var rows = [];
  for (var i = 0; i < INV_ENTRY.length; i++) {
    var r = INV_ENTRY[i];
    ...
    for (var j = 0; j < r.items.length; j++) {
      ...
      matched.forEach(function (b) {
        ...
        if (matched.length > 1) {
          for (var m = 0; m < r.items.length; m++) {
            var x = r.items[m];
            if (x.code === it.code && (x.pdate || '') === (b.pdate || '')) { src = x; break; }
          }
        }
        // 退货只从仓库出库：可退库存按仓库维度取 batch.w，与提交扣减口径一致
        var cur = b.w;
        var already = INV_RT_EDIT_ITEMS.some(function (x2) { return x2.rkNo === r.no && x2.batch === b.no && x2.code === it.code; });
        if (already || cur <= 0) return;
        rows.push({ rkNo: r.no, batch: b.no, name: it.name, code: it.code, spec: it.spec || '', rkQty: src.qty, cur: cur, cost: src.price, pdate: b.pdate || '', shelfLife: b.shelfLife != null ? b.shelfLife : src.shelfLife, shelfLifeType: b.shelfLifeType || 1, exp: b.exp || '' });
      });
    }
  }
  if (!rows.length) { msInvToast('供应商「' + sup + '」暂无已完成入库单批次可退'); return; }
  window._rtCand = rows;
  // 方式切换（轻量文本 tabs）：按批次=逐批勾选；先进先出=按生产日期从早到晚自动分摊到批次
  var tabs = '<div style="display:flex;align-items:center;gap:16px;margin-bottom:8px">' +
    '<span style="font-size:13px;font-weight:600;color:#0b1019">选择退货来源</span>' +
    '<a style="font-size:12px;cursor:pointer;' + (mode === 'batch' ? 'color:#005cf5;font-weight:600' : 'color:#5b6472') + '" onclick="invRtPickGoods(\'batch\')">按批次</a>' +
    '<a style="font-size:12px;cursor:pointer;' + (mode === 'fifo' ? 'color:#005cf5;font-weight:600' : 'color:#5b6472') + '" onclick="invRtPickGoods(\'fifo\')">先进先出</a>' +
    '<span style="flex:1"></span><span style="font-size:12px;color:#8a93a3">供应商：<b style="color:#0b1019">' + msInvEsc(sup) + '</b></span></div>';
  if (mode === 'fifo') {
    // 按商品聚合：可退总量 = Σ min(该批入库数量, 批次仓库库存)，排除已在明细中的批次
    var aggMap = {}, agg = [];
    rows.forEach(function (x) {
      var a = aggMap[x.code];
      if (!a) { a = aggMap[x.code] = { name: x.name, code: x.code, spec: x.spec, total: 0, batches: [] }; agg.push(a); }
      var cap = Math.min(x.rkQty, x.cur);
      var already = INV_RT_EDIT_ITEMS.some(function (x2) { return x2.rkNo === x.rkNo && x2.batch === x.batch && x2.code === x.code; });
      if (already || cap <= 0) return;
      a.total += cap; a.batches.push(x);
    });
    agg.forEach(function (a) {
      a.batches.sort(function (p, q2) { return msInvPdateVal(p.pdate) - msInvPdateVal(q2.pdate) || (p.rkNo < q2.rkNo ? -1 : 1); });
      a.range = (a.batches[0].pdate || '—') + ' → ' + (a.batches[a.batches.length - 1].pdate || '—');
      var pmin = a.batches[0].cost, pmax = a.batches[0].cost;
      a.batches.forEach(function (b2) { if (b2.cost < pmin) pmin = b2.cost; if (b2.cost > pmax) pmax = b2.cost; });
      a.costText = pmin === pmax ? pmin.toFixed(2) : pmin.toFixed(2) + ' ~ ' + pmax.toFixed(2);
      a.costDiff = pmin !== pmax;
    });
    window._rtFifoAgg = agg;
    if (!agg.length) { msInvToast('该供应商暂无可退批次（或均已加入明细）'); return; }
    var flist = agg.map(function (a, i) {
      return '<tr><td style="text-align:center;color:#999">' + (i + 1) + '</td><td>' + msInvEsc(a.name) + '</td><td>' + msInvEsc(a.code) + '</td><td>' + msInvEsc(a.spec || '—') + '</td>' +
        '<td style="text-align:right;font-weight:600;color:#1677ff">' + a.total + '</td>' +
        '<td style="width:120px"><input type="number" id="rtfq_' + i + '" min="0" max="' + a.total + '" placeholder="填数量" style="width:100px;height:28px;border:1px solid #dfe3ed;border-radius:3px;padding:0 8px;font-size:12px"></td>' +
        '<td style="text-align:right;' + (a.costDiff ? 'color:#f57316;font-weight:600' : 'color:#5b6472') + ';white-space:nowrap">' + a.costText + (a.costDiff ? '<div style="font-size:10px;color:#8a93a3;font-weight:400">跨批不同价</div>' : '') + '</td>' +
        '<td style="font-size:11px;color:#5b6472;white-space:nowrap">' + a.range + '（' + a.batches.length + ' 批）</td></tr>';
    }).join('');
    msInvModal({ title: '选择退货来源（先进先出）', width: 'min(980px,96vw)', body:
      tabs +
      '<div style="font-size:12px;color:#5b6472;margin-bottom:8px">填退货数量即可，系统按<b style="color:#0b1019">生产日期从早到晚</b>自动分摊到各批次（先产先退）；各批按<b style="color:#0b1019">自己的进货价</b>核算，分摊后在明细中可逐批人工修改退货价（不得高于该批进价）。</div>' +
      '<style>.icm-tb th{padding:9px 12px !important}.icm-tb td{padding:9px 12px !important}</style>' +
      '<div class="icm-tb" style="max-height:420px;overflow:auto"><table style="width:100%;min-width:820px">' +
      '<thead><tr><th style="width:40px">序号</th><th>商品名称</th><th>编码/条码</th><th style="width:90px">规格</th><th style="width:80px">可退总量</th><th style="width:120px">退货数量</th><th style="width:100px">进货价(元)</th><th>生产日期范围</th></tr></thead>' +
      '<tbody>' + flist + '</tbody></table></div>',
      onOk: 'invRtPickFifoOk()', okText: '按先进先出分摊' });
    return;
  }
  var list = rows.map(function (x, i) {
    var maxQ = Math.min(x.rkQty, x.cur);
    var remain = x.exp ? msInvRemainDays(x.exp) : null;
    var remHtml = remain == null ? '' : ' <span style="color:' + (remain <= 0 ? '#ec2d30' : (remain <= 7 ? '#f57316' : '#3eb27e')) + '">(' + remain + '天)</span>';
    return '<tr><td style="text-align:center"><input type="checkbox" id="rtpick_' + i + '"></td><td>' + msInvEsc(x.name) + '</td><td>' + msInvEsc(x.code) + '</td>' +
      ...
      '<td style="font-size:11px">' + msInvEsc(x.batch) + '</td>' +
      '<td style="text-align:right">' + x.rkQty + '</td><td style="text-align:right">' + x.cur + '</td><td style="text-align:right;color:#1677ff">' + maxQ + '</td>' +
      '<td style="text-align:right">' + x.cost.toFixed(2) + '</td><td style="font-size:11px;color:#5b6472;white-space:nowrap">' + (x.pdate || '—') + ' → ' + (x.exp || '—') + (x.shelfLife != null ? '<div style="color:#8a93a3">保质期 ' + msInvLifeText(x) + '</div>' : '') + remHtml + '</td></tr>';
  }).join('');
  msInvModal({ title: '选择退货来源（按批次）', width: 'min(1180px,96vw)', body:
    tabs +
    '<div style="font-size:12px;color:#5b6472;margin-bottom:8px">同单同商品多生产日期已按批次拆分（批次号带 -N 后缀）；勾选批次加入明细，可退上限 = min(该批入库数量, 当前批次仓库库存)</div>' +
    '<style>.icm-tb th{padding:9px 12px !important}.icm-tb td{padding:9px 12px !important}</style>' +
    '<div class="icm-tb" style="max-height:420px;overflow:auto"><table style="width:100%;min-width:1080px">' +
    '<thead><tr><th style="width:40px"></th><th>商品名称</th><th>编码/条码</th><th style="width:150px">入库单号</th><th style="width:130px">批次号</th><th style="width:80px">入库数量</th><th style="width:80px">仓库库存</th><th style="width:80px">最大可退</th><th style="width:80px">进货价</th><th style="width:190px">效期（生产日期 → 到期日）</th></tr></thead>' +
...
function invRtPickOk() {
  ...
  cand.forEach(function (x, i) {
    ...
    if (cb && cb.checked) {
      INV_RT_EDIT_ITEMS.push({ name: x.name, code: x.code, spec: x.spec, rkNo: x.rkNo, batch: x.batch, pdate: x.pdate, shelfLife: x.shelfLife, shelfLifeType: x.shelfLifeType, exp: x.exp, rkQty: x.rkQty, cur: x.cur, qty: 1, cost: x.cost, price: x.cost });
      added++;
    }
  });
...
// 先进先出分摊确认：按生产日期从早到晚把各商品填写数量分摊到批次（每批上限 = min(入库数量, 仓库库存)）
// 两遍法：先全量校验（超上限/无可分摊批次），全部通过后再统一应用，避免部分写入后拦截造成重复行
function invRtPickFifoOk() {
  var agg = window._rtFifoAgg || [], plans = [], err = '';
  agg.forEach(function (a, i) {
    var el = document.getElementById('rtfq_' + i);
    var q = el ? parseFloat(el.value) : 0;
    if (!q || q <= 0) return;
    if (q > a.total) { err += a.name + '（最大可退 ' + a.total + '） '; return; }
    var rem = q;
    var rowPlans = [];
    for (var bi = 0; bi < a.batches.length && rem > 0; bi++) {
      var b = a.batches[bi];
      var cap = Math.min(b.rkQty, b.cur);
      if (cap <= 0) continue;
      if (INV_RT_EDIT_ITEMS.some(function (x2) { return x2.rkNo === b.rkNo && x2.batch === b.batch && x2.code === b.code; })) continue;
      var alloc = Math.min(cap, rem);
      rowPlans.push({ b: b, alloc: alloc });
      rem -= alloc;
    }
    if (rem > 0.0001) { err += a.name + '（可分摊批次不足） '; return; }
    plans = plans.concat(rowPlans);
  });
  if (err) { msInvToast('无法按填写数量完成分摊：' + err + '，请修正'); return; }
  if (!plans.length) { msInvToast('请至少为一个商品填写退货数量'); return; }
  plans.forEach(function (p) {
    var b = p.b;
    INV_RT_EDIT_ITEMS.push({ name: b.name, code: b.code, spec: b.spec, rkNo: b.rkNo, batch: b.batch, pdate: b.pdate, shelfLife: b.shelfLife, shelfLifeType: b.shelfLifeType, exp: b.exp, rkQty: b.rkQty, cur: b.cur, qty: p.alloc, cost: b.cost, price: b.cost });
  });
  msInvCloseModal(); invRtEditRender();
}
...
function invRtView(no) {
  ...
  var rows = r.items.map(function (it, i) {
    ...
    return '<tr><td style="text-align:center;color:#999">' + (i + 1) + '</td><td>' + msInvEsc(it.name) + '</td><td>' + msInvEsc(it.code) + '</td>' +
      ...
      '<td style="font-size:11px">' + msInvEsc(it.batch || it.rkNo) + '</td>' +
      '<td style="text-align:right">' + it.qty + '</td><td style="text-align:right">¥' + it.price.toFixed(2) + '</td><td style="text-align:right">¥' + (it.qty * it.price).toFixed(2) + '</td>' +
      '<td style="font-size:12px;white-space:nowrap">' + (it.pdate || '—') + '</td>' +
      '<td style="font-size:12px;white-space:nowrap">' + (it.exp || '—') + '</td>' +
      '<td style="font-size:12px;white-space:nowrap">' + remHtml + '</td></tr>';
  }).join('');
  ...
  msInvModal({ title: '退货单详情 · ' + r.no, width: 'min(1280px,96vw)', body:
    ...
    '</div>' +
    '<style>.icm-tb th{padding:9px 12px !important}.icm-tb td{padding:9px 12px !important}</style>' +
    '<div class="icm-tb" style="max-height:340px;overflow:auto"><table style="width:100%;min-width:920px"><thead><tr><th style="width:50px">序号</th><th>商品名称</th><th>编码/条码</th><th style="width:140px">入库单号</th><th style="width:130px">批次号</th><th style="width:70px">数量</th><th style="width:80px">退货价</th><th style="width:90px">金额</th><th style="width:100px">生产日期</th><th style="width:100px">到期日</th><th style="width:96px">剩余有效期</th></tr></thead><tbody>' + rows + '</tbody></table></div>' + footBtn,
    onOk: 'msInvCloseModal()', okText: '关闭', cancelText: '' });
}
// 打印退货单（A4）
...
var INV_CK_KEY = 'tcm_inv_check_v9';
function invCkSeed() {
  ...
  // 效期商品按批次逐行展开（多效期=多行）：每行 snapshot=该批次账面，各批次行合计=商品总账面；每批独立实盘/差异。
  // 非效期行 snapshot=商品总账面（与 INV_GOODS_SEED 的 w/s 合计一致），保证「批次/总数口径一致」。
  return [
    { no: 'PD20260912001', date: '2026-09-12', endDate: '2026-09-12', storeId: 'S2001', loc: '仓库', mode: '锁库', by: '陈明', diff: 2, diffQty: 3, diffAmt: 30.1, status: 'done', items: [
      { name: '娃娃菜', code: '6901234500017', spec: '500g/份', unit: '份', cost: 2.8, expiryManaged: false, snapshot: 100, real: 98, diff: 2 },
      { name: '五花肉', code: '6901234500062', spec: '称重', unit: 'kg', cost: 24.5, expiryManaged: true, batchNo: 'RK20260910003', pdate: '2026-09-10', exp: '2026-09-13', remain: 1, snapshot: 30, real: 31, diff: 1 },
      { name: '五花肉', code: '6901234500062', spec: '称重', unit: 'kg', cost: 24.5, expiryManaged: true, batchNo: 'RK20260906003', pdate: '2026-09-06', exp: '2026-09-09', remain: -3, snapshot: 12, real: 12, diff: 0 }
    ] },
    { no: 'PD20260912002', date: '2026-09-12', endDate: '', storeId: 'S2001', loc: '货架', mode: '动态', by: '刘洋', diff: 1, diffQty: 2, diffAmt: 47.6, status: 'checking', items: [
      { name: '光明鲜牛奶', code: '6901234500116', spec: '950ml/盒', unit: '盒', cost: 11.9, expiryManaged: true, batchNo: 'RK20260912005-2', pdate: '2026-09-09', exp: '2026-09-16', remain: 3, snapshot: 14, move: -1, book: 13, real: 14, diff: -1 },
      { name: '光明鲜牛奶', code: '6901234500116', spec: '950ml/盒', unit: '盒', cost: 11.9, expiryManaged: true, batchNo: 'RK20260912005', pdate: '2026-09-12', exp: '2026-09-19', remain: 6, snapshot: 10, move: 2, book: 12, real: 9, diff: 3 }
    ] },
    { no: 'PD20260912003', date: '2026-09-12', endDate: '2026-09-12', storeId: 'S2002', loc: '货架', mode: '锁库', by: '黄丽', diff: 1, diffQty: -3, diffAmt: 22.8, status: 'done', items: [
      { name: '红富士苹果', code: '6901234500093', spec: '称重', unit: 'kg', cost: 7.6, expiryManaged: true, batchNo: 'RK20260905001', pdate: '2026-09-05', exp: '2026-09-20', remain: 8, snapshot: 40, real: 42, diff: -2 },
      { name: '红富士苹果', code: '6901234500093', spec: '称重', unit: 'kg', cost: 7.6, expiryManaged: true, batchNo: 'RK20260822001', pdate: '2026-08-22', exp: '2026-09-06', remain: -6, snapshot: 8, real: 9, diff: -1 },
      { name: '土鸡蛋', code: '6901234500079', spec: '30枚/盒', unit: '盒', cost: 18.8, expiryManaged: true, batchNo: 'RK20260830001', pdate: '2026-08-30', exp: '2026-10-14', remain: 32, snapshot: 22, real: 22, diff: 0 }
    ] },
    { no: 'PD20260911001', date: '2026-09-11', endDate: '2026-09-11', storeId: 'S2001', loc: '货架', mode: '速盘', by: '李娜', diff: 1, diffQty: -3, diffAmt: 39.6, status: 'done', items: [
      { name: '草鱼', code: '6901234500086', spec: '称重', unit: 'kg', cost: 13.2, expiryManaged: true, batchNo: 'RK20260911004', pdate: '2026-09-11', exp: '2026-09-13', remain: 2, snapshot: 10, real: 12, diff: -2 },
      { name: '草鱼', code: '6901234500086', spec: '称重', unit: 'kg', cost: 13.2, expiryManaged: true, batchNo: 'RK20260908004', pdate: '2026-09-08', exp: '2026-09-10', remain: -1, snapshot: 2, real: 3, diff: -1 },
      { name: '西红柿', code: '6901234500055', spec: '称重', unit: 'kg', cost: 5.1, expiryManaged: false, snapshot: 18, real: 18, diff: 0 }
    ] },
    { no: 'PD20260910001', date: '2026-09-10', endDate: '2026-09-10', storeId: 'S2001', loc: '仓库', mode: '动态', by: '陈明', diff: 1, diffQty: -8, diffAmt: 12.8, status: 'done', items: [
      { name: '土豆', code: '6901234500048', spec: '称重', unit: 'kg', cost: 3.4, expiryManaged: false, snapshot: 200, move: -6, book: 194, real: 194, diff: 0 },
      { name: '白萝卜', code: '6901234500031', spec: '称重', unit: 'kg', cost: 1.6, expiryManaged: false, snapshot: 64, move: -5, book: 59, real: 67, diff: -8 }
    ] },
    { no: 'PD20260909001', date: '2026-09-09', endDate: '2026-09-09', storeId: 'S2002', loc: '仓库', mode: '锁库', by: '黄丽', diff: 1, diffQty: -2, diffAmt: 42.0, status: 'done', items: [
      { name: '思念水饺', code: '6901234500123', spec: '1kg/袋', unit: '袋', cost: 21.5, expiryManaged: true, batchNo: 'RK20260818001', pdate: '2026-08-18', exp: '2027-02-14', remain: 158, snapshot: 18, real: 20, diff: -2 },
      { name: '土鸡蛋', code: '6901234500079', spec: '30枚/盒', unit: '盒', cost: 18.8, expiryManaged: true, batchNo: 'RK20260830001', pdate: '2026-08-30', exp: '2026-10-14', remain: 35, snapshot: 35, real: 35, diff: 0 }
    ] },
    { no: 'PD20260908001', date: '2026-09-08', endDate: '2026-09-08', storeId: 'S2001', loc: '仓库', mode: '速盘', by: '王芳', diff: 1, diffQty: 4, diffAmt: 11.2, status: 'done', items: [
      { name: '娃娃菜', code: '6901234500017', spec: '500g/份', unit: '份', cost: 2.8, expiryManaged: false, snapshot: 100, real: 96, diff: 4 },
      { name: '金龙鱼调和油', code: '6901234500109', spec: '5L/瓶', unit: '瓶', cost: 62.0, expiryManaged: false, snapshot: 12, real: 12, diff: 0 }
    ] },
    ...
    { no: 'PD20260903002', date: '2026-09-03', endDate: '2026-09-03', storeId: 'S2001', loc: '货架', mode: '动态', by: '陈明', diff: 0, diffQty: 0, diffAmt: 0, status: 'done', items: [
      { name: '鲜鸡蛋', code: '6901234500087', spec: '30枚/盒', unit: '盒', cost: 3.2, expiryManaged: false, snapshot: 58, move: 1, book: 59, real: 59, diff: 0 },
      { name: '西红柿', code: '6901234500055', spec: '称重', unit: 'kg', cost: 5.1, expiryManaged: false, snapshot: 18, move: -2, book: 16, real: 16, diff: 0 }
    ] },
    ...
    { no: 'PD20260901001', date: '2026-09-01', endDate: '2026-09-01', storeId: 'S2002', loc: '仓库', mode: '锁库', by: '黄丽', diff: 1, diffQty: 1, diffAmt: 21.5, status: 'done', items: [
      { name: '土豆', code: '6901234500048', spec: '称重', unit: 'kg', cost: 3.4, expiryManaged: false, snapshot: 200, real: 200, diff: 0 },
      { name: '思念水饺', code: '6901234500123', spec: '1kg/袋', unit: '袋', cost: 21.5, expiryManaged: true, batchNo: 'RK20260818001', pdate: '2026-08-18', exp: '2027-02-14', remain: 155, snapshot: 18, real: 17, diff: 1 }
    ] }
  ];
}
var INV_CK = [];
...
/* 盘点快报（标记「规划」，功能保留可用）：按门店+日期每日一份，聚合当日全部盘点终值（完成+进行中），多次盘点自动取最终值。v1.4 升级为与页面同宽的全尺寸弹窗 */
var INV_RPT_SHOP = 'S2001', INV_RPT_DATE = '';
function invCkRptOpen() {
  // v1.4 标记「规划」：功能保留可用（全尺寸弹窗版），标仅表示该功能点处于规划打磨阶段
  invCkRptPickDefaults();
  var sz = invCheckModalSize();
  msInvModal({ title: '盘点快报（每日一份 · 当日终值）', width: sz.w, height: sz.h, bodyStyle: 'overflow:hidden;', footer: false, body:
    '<div style="display:flex;flex-direction:column;height:100%">' +
    '<div style="display:flex;gap:10px;align-items:center;margin-bottom:12px;flex-shrink:0">' +
      '<span style="font-size:12px;color:#3a4252">门店：</span>' +
      '<select class="ic-search" style="flex:0 0 200px" id="invRptShop" onchange="invCkRptSetShop(this.value)"><option value="S2001">崧泽-青浦旗舰店</option><option value="S2002">崧泽-松江分店</option></select>' +
    ...
    '</div>' +
    '<div id="invRptBody" style="flex:1;min-height:0;overflow:auto"></div>' +
    '</div>' });
...
function invCkRptRender() {
  ...
  if (!INV_RPT_DATE) { el.innerHTML = '<div style="text-align:center;color:#8a93a3;padding:60px 0;font-size:12px">该门店暂无盘点记录</div>'; return; }
  var a = invCkRptAgg();
  var TH = 'style="text-align:LEFT;padding:9px 12px;background:#f7f9fd;border-bottom:1px solid #e9eef7;font-weight:500;color:#3a4252"';
  var TD = 'style="padding:9px 12px;border-bottom:1px solid #f0f3fa"';
  var h = '';
  // KPI 卡：4 列网格，铺满宽度
  ...
  var cards = [
    ...
    { k: '覆盖商品', v: a.goodsCnt + ' 种', sub: '有差异 ' + a.diffCnt + ' 种' },
    { k: '净差异量', v: (a.netQty > 0 ? '+' : '') + a.netQty, sub: '盘盈为正 / 盘亏为负', c: a.netQty > 0 ? '#fc4b52' : (a.netQty < 0 ? '#3eb27e' : '#3a4252') },
    { k: '净差异金额', v: (a.netAmt > 0 ? '+' : (a.netAmt < 0 ? '-' : '')) + '¥' + Math.abs(a.netAmt).toFixed(2), sub: '按 |差异| × 单位成本 计', c: a.netAmt > 0 ? '#fc4b52' : (a.netAmt < 0 ? '#3eb27e' : '#3a4252') }
  ];
  cards.forEach(function (cd) {
    h += '<div style="background:#f7f9fd;border:1px solid #e9eef7;border-radius:4px;padding:14px 18px">' +
      '<div style="font-size:11px;color:#8a93a3">' + cd.k + '</div>' +
      '<div style="font-size:20px;font-weight:700;margin:4px 0 2px;' + (cd.c ? 'color:' + cd.c : 'color:#0b1019') + '">' + cd.v + '</div>' +
      '<div style="font-size:11px;color:#8a93a3">' + cd.sub + '</div></div>';
  });
  h += '</div>';
  // 过程修正：表格式呈现
  var fixed = a.rows.filter(function (r) { return r.fixed && r.firstDiff !== r.diff; });
  if (fixed.length) {
    h += '<div style="font-size:12px;font-weight:600;color:#3a4252;margin-bottom:6px">⚠️ 过程修正（当日同商品多次盘点，差异已纠正）</div>';
    h += '<table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:16px"><thead><tr>' +
      '<th ' + TH.replace('LEFT', 'left') + ' style2>商品</th>'.replace(' style2', '') +
      '<th ' + TH.replace('LEFT', 'right') + '>首次差异</th>' +
      '<th ' + TH.replace('LEFT', 'right') + '>终值差异</th>' +
      '<th ' + TH.replace('LEFT', 'left') + '>终值来源</th></tr></thead><tbody>';
    h += fixed.map(function (r) {
      var c1 = r.firstDiff > 0 ? '#fc4b52' : (r.firstDiff < 0 ? '#3eb27e' : '#909399');
      var c2 = r.diff > 0 ? '#fc4b52' : (r.diff < 0 ? '#3eb27e' : '#909399');
      return '<tr>' +
        '<td ' + TD + '>' + msInvEsc(r.name) + '</td>' +
        '<td ' + TD + ' style2="text-align:right;font-weight:600;color:' + c1 + '">'.replace(' style2', ' style') + (r.firstDiff > 0 ? '+' : '') + r.firstDiff + '</td>' +
        '<td ' + TD + ' style="text-align:right;font-weight:600;color:' + c2 + '">' + (r.diff > 0 ? '+' : '') + r.diff + '</td>' +
        '<td ' + TD + ' style="color:#5b6472">' + r.mode + ' ' + r.doc + (r.status === 'checking' ? ' <span style="color:#f57316">盘点中</span>' : '') + '</td></tr>';
    }).join('');
    h += '</tbody></table>';
  }
  // 盈亏明细：增列 规格 / 单位成本，列宽定宽对齐
  h += '<div style="font-size:12px;font-weight:600;color:#3a4252;margin-bottom:6px">盈亏明细（按差异金额排序）</div>';
  h += '<table style="width:100%;table-layout:fixed;border-collapse:collapse;font-size:12px"><thead><tr>' +
    '<th ' + TH.replace('LEFT', 'left') + ' width2>'.replace(' width2', ' width="16%"') + '商品</th>' +
    '<th ' + TH.replace('LEFT', 'left') + ' width="14%">编码</th>' +
    '<th ' + TH.replace('LEFT', 'left') + ' width="12%">规格</th>' +
    '<th ' + TH.replace('LEFT', 'right') + ' width="10%">单位成本</th>' +
    '<th ' + TH.replace('LEFT', 'right') + ' width="10%">差异量</th>' +
    '<th ' + TH.replace('LEFT', 'right') + ' width="12%">差异金额</th>' +
    '<th ' + TH.replace('LEFT', 'left') + '>终值来源</th></tr></thead><tbody>';
  if (!a.rows.length) h += '<tr><td colspan="7" style="text-align:center;color:#8a93a3;padding:40px 0">当日无盘点数据</td></tr>';
  a.rows.forEach(function (r) {
    var c = r.diff > 0 ? '#fc4b52' : (r.diff < 0 ? '#3eb27e' : '#909399');
    h += '<tr>' +
      '<td ' + TD + ' style="font-weight:600;color:#0b1019">' + msInvEsc(r.name) + '</td>' +
      '<td ' + TD + ' style="color:#8a93a3">' + msInvEsc(r.code) + '</td>' +
      '<td ' + TD + ' style="color:#5b6472">' + msInvEsc(r.spec || '—') + '</td>' +
      '<td ' + TD + ' style="text-align:right;color:#5b6472">¥' + (r.cost || 0).toFixed(2) + '</td>' +
      '<td ' + TD + ' style="text-align:right;font-weight:600;color:' + c + '">' + (r.diff > 0 ? '+' : '') + r.diff + '</td>' +
      '<td ' + TD + ' style="text-align:right;color:' + c + '">' + (r.amt ? (r.diff > 0 ? '+' : '-') + '¥' + r.amt.toFixed(2) : '¥0.00') + '</td>' +
      '<td ' + TD + ' style="color:#5b6472">' + r.mode + ' ' + r.doc + (r.status === 'checking' ? ' <span style="color:#f57316">盘点中(实时)</span>' : '') + '</td></tr>';
  });
  h += '</tbody></table>';
  el.innerHTML = h;
}
function invCkInit() {
  ...
  el.innerHTML =
    ...
    '<div style="flex-shrink:0;padding:8px 20px;background:#fff;border-bottom:1px solid #dfe3ed;display:flex;align-items:center;gap:8px;flex-wrap:wrap">' +
      ...
      '<button class="ic-btn" onclick="invCkRptOpen()">📋 盘点快报 <span style="display:inline-block;font-size:10px;color:#5b6472;background:#f1f2f5;border:1px solid #dfe3ed;border-radius:2px;padding:0 4px;margin-left:2px;vertical-align:1px">规划</span></button>' +
    ...
    for (var i = 0; i < INV_GOODS.length; i++) {
      ...
      if (g.expiryManaged && g.batches && g.batches.length) {
        g.batches.forEach(function (b) {
          ...
          if (bq < 0) return; // 系统批次全部展开（含零库存，按 0 展示，日期只读）
...
function invCkIsBatch(it) { return !!it.expiryManaged; }
...
function invCkAddBatchRow(code) {
  ...
  var spec = (g.unit === 'kg' || g.unit === 'l') ? '称重/' + g.unit : g.spec + '/' + g.unit;
  var row = { name: g.name, code: g.code, spec: spec, unit: g.unit, cost: g.avgCost, expiryManaged: true, pdate: '', exp: '', batchNo: '', remain: null, snapshot: 0, real: 0 };
  var last = -1;
  for (var i = 0; i < INV_CK_EDIT.length; i++) { if (INV_CK_EDIT[i].code === code) last = i; }
  if (last > -1) INV_CK_EDIT.splice(last + 1, 0, row); else INV_CK_EDIT.push(row);
...
function invCkSetPdate(i, v) {
  ...
  // P2 修复：按生产日期+保质期推算出的到期日早于今天时拦截（已过期日期不允许建批入库）
  if (it.exp && msInvRemainDays(it.exp) < 0) {
    it.exp = ''; var eInp0 = document.getElementById('ckexp_' + i); if (eInp0) eInp0.value = '';
    var remEl0 = document.getElementById('ckrem_' + i); if (remEl0) remEl0.textContent = '';
    msInvToast('该生产日期按保质期推算的到期日早于今天，已过期日期不允许建批入库，请核对生产日期'); return;
  }
...
function invCkSetExp(i, v) {
  ...
  // P2 修复：补录到期日不能早于今天（系统批次日期只读，此校验仅作用于「+批」新增批次）
  if (v && msInvRemainDays(v) < 0) {
    var el0 = document.getElementById('ckexp_' + i); if (el0) el0.value = it.exp || '';
    msInvToast('补录到期日不能早于今天：已过期批次应走报损/退货处理，不允许建批入库'); return;
  }
  it.exp = v;
...
function invCkAddGoodsConfirm() {
  ...
  function pushRow(b, pd, ex, snap) {
    var row = { name: g.name, code: g.code, spec: (g.unit === 'kg' || g.unit === 'l') ? '称重/' + g.unit : g.spec + '/' + g.unit, unit: g.unit, cost: g.avgCost, expiryManaged: b, pdate: pd || '', exp: ex || '', batchNo: b ? b.no : '', remain: ex ? msInvRemainDays(ex) : null, snapshot: snap, real: snap };
    var last = -1;
    for (var i = 0; i < INV_CK_EDIT.length; i++) { if (INV_CK_EDIT[i].code === g.code) last = i; }
    if (last > -1) INV_CK_EDIT.splice(last + 1, 0, row); else INV_CK_EDIT.push(row);
  }
...
function invCkEditRender() {
  ...
  // 商品分组渲染：名称/编码/规格/计数方式/快照/变动 每商品一组（rowspan），效期及之后每效期一行
  var rowsHtml = '', groupIdx = 0, gi = 0;
  var isLock = window._ckMode === '锁库';
  while (gi < INV_CK_EDIT.length) {
    var gCode = INV_CK_EDIT[gi].code, gN = 0, gSnap = 0, gLive = 0, gGoods = msInvFindGoods(gCode);
    for (var gj = gi; gj < INV_CK_EDIT.length && INV_CK_EDIT[gj].code === gCode; gj++) {
      gN++;
      gSnap += INV_CK_EDIT[gj].snapshot;
      if (gGoods && gGoods.storeId === window._ckStore) gLive += invCkIsBatch(INV_CK_EDIT[gj]) ? invCkBatchQty(gGoods, window._ckLoc, INV_CK_EDIT[gj].batchNo) : invCkLiveQty(gGoods, window._ckLoc);
    }
    var it0 = INV_CK_EDIT[gi];
    var modeCell = invCkIsBatch(it0)
      ? '<span style="display:inline-block;font-size:11px;color:#0b7a52;background:#e8f7ef;border:1px solid #b7ebcf;border-radius:2px;padding:1px 6px;white-space:nowrap">按效期</span>'
      : '<span style="display:inline-block;font-size:11px;color:#5b6472;background:#f1f2f5;border:1px solid #dfe3ed;border-radius:2px;padding:1px 6px;white-space:nowrap">按总数</span>';
    var headTds =
      '<td rowspan="' + gN + '" style="text-align:center;vertical-align:middle;padding:8px 6px;color:#999">' + (groupIdx + 1) + '</td>' +
      '<td rowspan="' + gN + '" style="text-align:left;vertical-align:middle;padding:8px 10px;font-weight:600">' + msInvEsc(it0.name) + (invCkIsBatch(it0) ? ' <button class="ic-btn" title="漏盘补录：新增一行效期批次" style="height:20px;padding:0 6px;font-size:11px;vertical-align:middle;font-weight:400" onclick="invCkAddBatchRow(\'' + gCode + '\')">+批</button>' : '') + '</td>' +
      '<td rowspan="' + gN + '" style="text-align:left;vertical-align:middle;padding:8px 10px">' + msInvEsc(gCode) + '</td>' +
      '<td rowspan="' + gN + '" style="text-align:left;vertical-align:middle;padding:8px 10px">' + msInvEsc(it0.spec) + '</td>' +
      '<td rowspan="' + gN + '" style="text-align:left;vertical-align:middle;padding:8px 10px">' + modeCell + '</td>' +
      '<td rowspan="' + gN + '" style="text-align:right;vertical-align:middle;padding:8px 10px">' + (Math.round(gSnap * 100) / 100) + '</td>' +
      (isLock ? '' : '<td rowspan="' + gN + '" style="text-align:right;vertical-align:middle;padding:8px 10px;color:#1677ff">' + (Math.round((gLive - gSnap) * 100) / 100) + '</td>');
    for (var k = gi; k < gi + gN; k++) {
      var it = INV_CK_EDIT[k];
      var isBatch = invCkIsBatch(it);
      var live = 0;
      if (gGoods && gGoods.storeId === window._ckStore) live = isBatch ? invCkBatchQty(gGoods, window._ckLoc, it.batchNo) : invCkLiveQty(gGoods, window._ckLoc);
      ...
      if (isBatch) {
        var remain = it.exp ? msInvRemainDays(it.exp) : null;
        var remTxt = remain != null ? ' <span id="ckrem_' + k + '" style="font-size:11px;white-space:nowrap;color:' + (remain <= 0 ? '#ec2d30' : (remain <= 7 ? '#f57316' : '#3eb27e')) + '">(' + remain + '天)</span>' : ' <span id="ckrem_' + k + '"></span>';
        if (it.batchNo) {
          // 系统入库批次：日期只读（含零库存批，库存量按 0 展示），仅新增批次可改
          dateCell = '<span style="font-size:12px;white-space:nowrap" title="系统入库批次（' + it.batchNo + '），日期不可修改">' + (it.pdate || '—') + ' <span style="color:#8a93a3">→</span> ' + (it.exp || '—') + '</span>' + remTxt;
        } else {
          dateCell = '<span style="white-space:nowrap"><input type="date" id="ckpd_' + k + '" value="' + (it.pdate || '') + '" onchange="invCkSetPdate(' + k + ',this.value)" class="ic-search" style="width:88px;height:26px;font-size:11px"> <span style="color:#8a93a3">→</span> <input type="date" id="ckexp_' + k + '" value="' + (it.exp || '') + '" onchange="invCkSetExp(' + k + ',this.value)" class="ic-search" style="width:88px;height:26px;font-size:11px"></span>' + remTxt;
        }
      } else { dateCell = '—'; }
      rowsHtml += '<tr>' + headTds +
      (anyBatch ? '<td style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px;white-space:nowrap">' + dateCell + '</td>' : '') +
      '<td style="text-align:right;vertical-align:middle;padding:8px 10px">' + live + '</td>' +
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px"><input id="ckreal_' + k + '" type="number" min="0" class="ic-search" style="width:100%" value="' + it.real + '" oninput="invCkDiff(' + k + ')"></td>' +
      '<td style="text-align:right;vertical-align:middle;padding:8px 10px;font-weight:600;' + diffCls + '" id="ckdiff_' + k + '">' + (diff > 0 ? '+' : '') + diff + '</td>' +
      '<td style="text-align:right;vertical-align:middle;padding:8px 10px;font-weight:600;' + diffCls + '" id="ckamt_' + k + '">¥' + diffAmt.toFixed(2) + '</td>' +
      '</tr>';
      headTds = '';
    }
    groupIdx++;
  ...
  var body =
    ...
    '<div style="font-size:11px;color:#8a93a3;background:#f7f9fc;border:1px solid #eef1f6;border-radius:4px;padding:6px 10px;margin-bottom:8px;flex-shrink:0">' +
      ...
      ' 同单可混合有效期/非有效期商品（如食品与家具）；是否按效期由商品档案的效期管理开关决定（商品上设置，盘点时不可改），各按其方式录入、互不影响；' + (anyBatch ? '开启效期的商品已按各到期日逐批展开（系统批次日期只读），漏盘可点商品名称旁「+批」补日期。' : '') +
      ' 差异 = 账面 − 实盘；差异金额 = |差异量| × 单位成本。</div>' +
    '<div style="display:flex;gap:8px;align-items:center;margin-bottom:8px;flex-shrink:0"><button class="ic-btn" onclick="invCkAddGoodsOpen()">+ 添加商品</button><span style="flex:1"></span><span style="font-size:11px;color:#8a93a3">系统入库批次日期不可修改（无库存的批次按 0 展示）；仅「+批」新增批次可编辑日期，所选日期不在入库批次时提交自动建主数据批次</span></div>' +
    '<div style="flex:1;min-height:0;overflow:auto;border:1px solid #e9eef7;border-radius:4px"><table style="width:100%;table-layout:fixed;border-collapse:collapse">' +
      ...
      '<th style="width:5%;text-align:center;vertical-align:middle;padding:8px 6px">序号</th>' +
      '<th style="width:12%;text-align:left;vertical-align:middle;padding:8px 10px">商品名称</th>' +
      '<th style="width:10%;text-align:left;vertical-align:middle;padding:8px 10px">编码/条码</th>' +
      '<th style="width:7%;text-align:left;vertical-align:middle;padding:8px 10px">规格</th>' +
      '<th style="width:7%;text-align:left;vertical-align:middle;padding:8px 10px">计数方式</th>' +
      (anyBatch ? '<th style="width:17%;text-align:left;vertical-align:middle;padding:8px 10px">生产日期 → 到期日</th>' : '') +
      '<th style="width:7%;text-align:right;vertical-align:middle;padding:8px 10px">' + (isLock ? '冻结账面' : '开始快照') + '</th>' +
      (isLock ? '' : '<th style="width:6%;text-align:right;vertical-align:middle;padding:8px 10px">变动</th><th style="width:7%;text-align:right;vertical-align:middle;padding:8px 10px">理论库存</th>') +
      '<th style="width:8%;text-align:right;vertical-align:middle;padding:8px 10px">实盘数量</th>' +
      '<th style="width:7%;text-align:right;vertical-align:middle;padding:8px 10px">差异量</th>' +
      '<th style="width:7%;text-align:right;vertical-align:middle;padding:8px 10px">差异金额</th></tr></thead>' +
      '<tbody>' + rowsHtml + '</tbody>' +
    '</table></div>' +
    '</div>';
  var sz = invCheckModalSize();
...
function invCkCollect() {
  ...
  var items = INV_CK_EDIT.map(function (it, i) {
    ...
    return { name: it.name, code: it.code, spec: it.spec, unit: it.unit, cost: it.cost, expiryManaged: invCkIsBatch(it), pdate: it.pdate, exp: it.exp, batchNo: it.batchNo, snapshot: it.snapshot, move: Math.round((bookVal - it.snapshot) * 100) / 100, book: Math.round(bookVal * 100) / 100, real: real, diff: d };
  });
...
function invCkSaveCommit() {
  // P1 修复：漏盘补录校验——效期补录行（batchNo 为空）有差异但没填任何日期时，禁止提交（此前会静默丢单）
  var miss = [];
  for (var mi = 0; mi < INV_CK_EDIT.length; mi++) {
    var mt = INV_CK_EDIT[mi];
    if (!mt.expiryManaged || mt.batchNo) continue;
    if (!mt.pdate && !mt.exp && mt.real !== 0) miss.push(mt.name);
  }
  if (miss.length) { msInvToast('以下商品补录批次未填生产/到期日期，无法建批回写库存：' + miss.join('、') + '（请补日期或删除该行）'); return; }
  var past = [];
  for (var pi = 0; pi < INV_CK_EDIT.length; pi++) {
    var pt = INV_CK_EDIT[pi];
    if (!pt.expiryManaged || pt.batchNo) continue;
    if (pt.exp && msInvRemainDays(pt.exp) < 0) past.push(pt.name);
  }
  if (past.length) { msInvToast('以下商品补录到期日早于今天（不允许建批入库）：' + past.join('、') + '（已过期批次应走报损/退货）'); return; }
  var d = invCkCollect();
  var exists = false, idx = -1;
  INV_CK.forEach(function (x, i) { if (x.no === window._ckNo) { exists = true; idx = i; } });
  ...
  d.items.forEach(function (it) {
    ...
    if (it.expiryManaged) {
      ...
      // 漏盘补录：所选日期在入库批次中不存在 → 自动在主数据新建批次（pdate/exp/实盘），补全主数据并修正库存
      // P1 修复：只填到期日也能建批——生产日期缺省时按商品保质期自到期日反推；实盘为 0 的空补录行不建批
      if (!b && (it.pdate || it.exp) && it.real !== 0) {
        var pd = it.pdate;
        if (!pd && it.exp) { var slR = invCkGoodsShelfLife(it.code); if (slR) pd = invCkAddDays(it.exp, -slR); }
        var bno = 'RK' + (pd || it.exp || msInvToday()).replace(/-/g, '') + '-' + String(it.code).slice(-4) + (Math.floor(Math.random() * 90) + 10);
        b = { no: bno, pdate: pd || '', shelfLife: invCkGoodsShelfLife(it.code), shelfLifeType: 1, exp: it.exp || '', w: 0, s: 0 };
...
function invCkView(no, editMode) {
  ...
  if (!r) return;
  var storeName = { S2001: '崧泽-青浦旗舰店', S2002: '崧泽-松江分店' };
  var mode = invCkModeText(r.mode);
  var isDyn = mode === '动态';
  var anyExp = r.items.some(function (it) { return it.expiryManaged; });
  // 过程数据：快照(开始时点账面) → 变动(期间库存流水) → 账面(快照+变动) → 实盘 → 差异；按商品分组渲染，效期及之后每批一行
  var rowsHtml = '', groupIdx = 0, gi = 0;
  while (gi < r.items.length) {
    var gCode = r.items[gi].code, gN = 0;
    for (var gj = gi; gj < r.items.length && r.items[gj].code === gCode; gj++) gN++;
    var it0 = r.items[gi];
    var modeCell = it0.expiryManaged ? '<span style="display:inline-block;font-size:11px;color:#0b7a52;background:#e8f7ef;border:1px solid #b7ebcf;border-radius:2px;padding:1px 6px;white-space:nowrap">按效期</span>' : '<span style="display:inline-block;font-size:11px;color:#5b6472;background:#f1f2f5;border:1px solid #dfe3ed;border-radius:2px;padding:1px 6px;white-space:nowrap">按总数</span>';
    var headTds =
      '<td rowspan="' + gN + '" style="text-align:center;vertical-align:middle;padding:8px 6px;color:#999">' + (groupIdx + 1) + '</td>' +
      '<td rowspan="' + gN + '" style="text-align:left;vertical-align:middle;padding:8px 10px;font-weight:600">' + msInvEsc(it0.name) + '</td>' +
      '<td rowspan="' + gN + '" style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">' + msInvEsc(gCode) + '</td>' +
      '<td rowspan="' + gN + '" style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">' + msInvEsc(it0.spec || '') + '</td>' +
      '<td rowspan="' + gN + '" style="text-align:left;vertical-align:middle;padding:8px 10px">' + modeCell + '</td>';
    for (var k = gi; k < gi + gN; k++) {
      var it = r.items[k];
      var mv = it.move != null ? it.move : 0;
      var book = it.book != null ? it.book : Math.round((it.snapshot + mv) * 100) / 100;
      var diff = it.diff != null ? it.diff : Math.round((book - it.real) * 100) / 100;
      var dh = diff === 0 ? '<span style="color:#909399">0</span>' : (diff > 0 ? '<span style="color:#fc4b52">+' + diff + '</span>' : '<span style="color:#3eb27e">' + diff + '</span>');
      var amt = Math.round(Math.abs(diff) * (it.cost || 0) * 100) / 100;
      var mvCell = isDyn ? '<span style="color:#1677ff;font-weight:600">' + (mv > 0 ? '+' : '') + mv + '</span>' : '<span style="color:#c0c7d2">—</span>';
      ...
      rowsHtml += '<tr>' + headTds +
        ...
        '<td style="text-align:right;vertical-align:middle;padding:8px 10px">' + mvCell + '</td>' +
        '<td style="text-align:right;vertical-align:middle;padding:8px 10px;font-weight:600">' + book + '</td>' +
        '<td style="text-align:right;vertical-align:middle;padding:8px 10px">' + it.real + '</td>' +
        '<td style="text-align:right;vertical-align:middle;padding:8px 10px">' + dh + '</td>' +
  ...
  msInvModal({ title: '盘点单详情 · ' + r.no, width: sz.w, height: sz.h, bodyStyle: 'overflow:hidden;', body:
    ...
    '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;font-size:12px;color:#5b6472;margin-bottom:12px;flex-shrink:0">' +
      ...
      '<div>方式：<b style="color:#0b1019">' + mode + '</b></div><div>盘点人：<b style="color:#0b1019">' + msInvEsc(r.by) + '</b></div>' +
    ...
    '</div>' +
    '<div style="font-size:11px;color:#8a93a3;background:#f7f9fc;border:1px solid #eef1f6;border-radius:4px;padding:6px 10px;margin-bottom:8px;flex-shrink:0">' +
      '过程数据：快照 = 盘点开始时点账面；变动 = 期间库存流水（补货 / 销售 / 退仓，动态模式实时累计）；账面 = 快照 + 变动（锁库冻结、速盘即时回写，变动记 —）；差异 = 账面 − 实盘；差异金额 = |差异| × 单位成本。变动 / 账面为提交盘点时的落档值，不随后续库存变动。</div>' +
    '<div style="flex:1;min-height:0;overflow:auto"><table style="width:100%;table-layout:fixed;border-collapse:collapse"><thead><tr>' +
      ...
      '<th style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">编码/条码</th>' +
      '<th style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">规格</th>' +
      '<th style="text-align:left;vertical-align:middle;padding:8px 10px">计数方式</th>' +
      (anyExp ? '<th style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">到期日(有效期至)</th>' : '') +
      '<th style="text-align:right;vertical-align:middle;padding:8px 10px">快照</th>' +
      '<th style="text-align:right;vertical-align:middle;padding:8px 10px">变动</th>' +
      '<th style="text-align:right;vertical-align:middle;padding:8px 10px">账面</th>' +
      ...
      '<th style="text-align:right;vertical-align:middle;padding:8px 10px">差异金额</th></tr></thead><tbody>' + rowsHtml + '</tbody></table></div>' +
    '</div>',
    onOk: 'msInvCloseModal()', okText: '关闭', cancelText: '' });
}
...
function invCkSnapView(no) {
  ...
  var rows = r.items.map(function (it, i) {
    ...
    var diff = it.diff != null ? it.diff : Math.round((book - it.real) * 100) / 100;
    var dh = diff === 0 ? '<span style="color:#909399">0</span>' : (diff > 0 ? '<span style="color:#fc4b52">+' + diff + '</span>' : '<span style="color:#3eb27e">' + diff + '</span>');
    var mvHtml = '<span style="color:#1677ff;font-weight:600">' + (mv > 0 ? '+' : '') + mv + '</span>';
    var expCell = it.expiryManaged ? ((it.pdate ? it.pdate : '—') + ' → ' + (it.exp ? it.exp : '—')) : '—';
    return '<tr><td style="text-align:center;vertical-align:middle;padding:8px 6px;color:#999">' + (i + 1) + '</td>' +
      ...
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">' + msInvEsc(it.code) + '</td>' +
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">' + msInvEsc(it.spec || '') + '</td>' +
      (anyExp ? '<td style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">' + expCell + '</td>' : '') +
      '<td style="text-align:right;vertical-align:middle;padding:8px 10px">' + it.snapshot + '</td>' +
      '<td style="text-align:right;vertical-align:middle;padding:8px 10px">' + mvHtml + '</td>' +
      '<td style="text-align:right;vertical-align:middle;padding:8px 10px;font-weight:600">' + book + '</td>' +
      '<td style="text-align:right;vertical-align:middle;padding:8px 10px">' + it.real + '</td>' +
      '<td style="text-align:right;vertical-align:middle;padding:8px 10px">' + dh + '</td></tr>';
  }).join('');
  var sz = invCheckModalSize();
  msInvModal({ title: '盘点快照 · ' + r.no, width: sz.w, height: sz.h, bodyStyle: 'overflow:hidden;', body:
    ...
    '</div>' +
    '<div style="font-size:11px;color:#8a93a3;background:#f7f9fc;border:1px solid #eef1f6;border-radius:4px;padding:6px 10px;margin-bottom:8px;flex-shrink:0">' +
      '快照 = 盘点开始时点账面；变动 = 期间库存流水（补货 / 销售 / 退仓）；账面 = 快照 + 变动；差异 = 账面 − 实盘。变动 / 账面为提交盘点时的落档值。</div>' +
    '<div style="flex:1;min-height:0;overflow:auto"><table style="width:100%;table-layout:fixed;border-collapse:collapse"><thead><tr>' +
      '<th style="width:5%;text-align:center;vertical-align:middle;padding:8px 6px">序号</th>' +
      '<th style="text-align:left;vertical-align:middle;padding:8px 10px">商品名称</th>' +
      '<th style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">编码/条码</th>' +
...
function invCkQuickRenderReal() {
  ...
  // P3 修复：开效期但暂无批次的商品不可速盘——按总数回写会造成批次合计脱钩，提示走正式盘点「+批」补录
  if (g.expiryManaged && (!g.batches || !g.batches.length)) {
    realBox.innerHTML = '<div style="font-size:12px;color:#f57316;background:#fff7f0;border:1px solid #ffe0b8;border-radius:4px;padding:12px;line-height:20px">' + msInvEsc(g.name) + ' 已开启效期管理，但暂无入库批次，无法速盘回写。<br>请先完成该商品的入库，或改用「盘点记录」新增盘点单后点商品旁「+批」补录批次。</div>';
    return;
  }
  if (g.expiryManaged && g.batches && g.batches.length) {
...
function invCkQuickSubmit() {
  ...
  // P3 修复：提交端兜底——开效期但无批次不允许按总数回写
  if (g.expiryManaged && (!g.batches || !g.batches.length)) { msInvToast(g.name + ' 已开启效期管理但暂无批次，无法速盘，请先入库或走盘点单「+批」补录'); return; }
  var newItems = [];
  if (g.expiryManaged && g.batches && g.batches.length) {
    g.batches.forEach(function (b) {
      ...
      newItems.push({ name: g.name, code: g.code, spec: (g.unit === 'kg' || g.unit === 'l') ? '称重/' + g.unit : g.spec + '/' + g.unit, unit: g.unit, cost: g.avgCost, expiryManaged: true, batchNo: b.no, pdate: b.pdate || '', exp: b.exp || '', remain: msInvRemainDays(b.exp), snapshot: cur, move: 0, book: cur, real: real, diff: Math.round((cur - real) * 100) / 100 });
    });
  } else {
    ...
    newItems.push({ name: g.name, code: g.code, spec: (g.unit === 'kg' || g.unit === 'l') ? '称重/' + g.unit : g.spec + '/' + g.unit, unit: g.unit, cost: g.avgCost, expiryManaged: false, snapshot: cur, move: 0, book: cur, real: real, diff: Math.round((cur - real) * 100) / 100 });
  }
  ...
  order.items.forEach(function (it) { var d = it.diff != null ? it.diff : Math.round((it.snapshot - it.real) * 100) / 100; if (d !== 0) { if (!seen[it.code]) { seen[it.code] = 1; dc++; } dq += d; da += Math.round(Math.abs(d) * (it.cost || 0) * 100) / 100; } });
  ...
  el.innerHTML =
    ...
    '<div style="flex:1;min-height:0;margin:10px 10px 4px;background:#fff;border-radius:4px;display:flex;flex-direction:column;border:1px solid #e9eef7;overflow:hidden">' +
      '<div class="table-wrap" style="flex:1;overflow:auto;min-height:0"><table style="width:100%;table-layout:fixed;border-collapse:collapse">' +
        '<thead><tr>' +
          '<th style="width:4%;text-align:center;vertical-align:middle;padding:8px 6px">序号</th>' +
          '<th style="width:8%;text-align:left;vertical-align:middle;padding:8px 10px">商品名称</th>' +
          '<th style="width:9%;text-align:left;vertical-align:middle;padding:8px 10px">编码/条码</th>' +
          '<th style="width:6%;text-align:left;vertical-align:middle;padding:8px 10px">商品规格</th>' +
          '<th style="width:9%;text-align:left;vertical-align:middle;padding:8px 10px">入库单号</th>' +
          '<th style="width:8%;text-align:left;vertical-align:middle;padding:8px 10px">批次号</th>' +
          '<th style="width:9%;text-align:left;vertical-align:middle;padding:8px 10px">生产日期</th>' +
          '<th style="width:9%;text-align:left;vertical-align:middle;padding:8px 10px">有效期至</th>' +
          '<th style="width:6%;text-align:right;vertical-align:middle;padding:8px 10px">初始量</th>' +
          '<th style="width:6%;text-align:right;vertical-align:middle;padding:8px 10px">变动量</th>' +
          '<th style="width:6%;text-align:right;vertical-align:middle;padding:8px 10px">最新值</th>' +
          '<th style="width:7%;text-align:center;vertical-align:middle;padding:8px 6px">明细类型</th>' +
          '<th style="width:5%;text-align:center;vertical-align:middle;padding:8px 6px">点位</th>' +
          '<th style="width:8%;text-align:left;vertical-align:middle;padding:8px 10px">门店</th>' +
        '</tr></thead>' +
...
function invDetailRender() {
  ...
  tbody.innerHTML = data.length ? data.map(function (r, i) {
    ...
    // B8：按流水批次号反查主数据批次，补效期维度（生产日期/有效期至）；无批次或批次已不存在显示 —
    var gb = null;
    if (g && g.batches && r.batch) { for (var bi = 0; bi < g.batches.length; bi++) { if (g.batches[bi].no === r.batch) { gb = g.batches[bi]; break; } } }
    return '<tr style="height:42px">' +
      '<td style="text-align:center;vertical-align:middle;padding:8px 6px;color:#999">' + seq + '</td>' +
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px">' + msInvEsc(r.name) + '</td>' +
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">' + msInvEsc(r.code) + '</td>' +
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">' + msInvEsc(spec) + '</td>' +
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:11px">' + msInvEsc(r.refNo) + '</td>' +
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:11px">' + msInvEsc(r.batch) + '</td>' +
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">' + (gb ? (gb.pdate || '—') : '—') + '</td>' +
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">' + (gb ? (gb.exp || '—') : '—') + '</td>' +
      ...
      '<td style="text-align:left;vertical-align:middle;padding:8px 10px;font-size:12px">' + msInvEsc(store) + '</td>' +
    '</tr>';
  }).join('') : '<tr><td colspan="14" style="text-align:center;color:#999;padding:36px 0">暂无变动流水</td></tr>';
