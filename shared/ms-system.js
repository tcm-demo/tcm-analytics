...
var SY_MENU_SEED = [
  ...
  { id: 'm006', pid: '', code: '006', name: '库存管理', path: '', web: true, apk: false, mp: true, level: 1, icon: '', tags: '规划', status: 0, children: [
    { id: 'm00601', pid: 'm006', code: '006001', name: '库存查询', path: '/inv-list', web: true, apk: false, mp: true, level: 2, icon: '', tags: '', status: 0, children: [] },
    { id: 'm00602', pid: 'm006', code: '006002', name: '入库记录', path: '/inv-entry', web: true, apk: false, mp: false, level: 1, icon: '', tags: '', status: 0, children: [] },
    { id: 'm00611', pid: 'm006', code: '006011', name: '采购定价', path: '/supply-price', web: true, apk: false, mp: false, level: 1, icon: '', tags: '', status: 0, children: [] },
    { id: 'm00612', pid: 'm006', code: '006012', name: '价格审批', path: '/price-approval', web: true, apk: false, mp: false, level: 1, icon: '', tags: '', status: 0, children: [] },
...
function syMenuLoad() {
  try { var r = localStorage.getItem(SY_MENU_KEY); if (r) { SY_MENU = JSON.parse(r); syMenuSeedMerge(); return; } } catch (e) {}
  SY_MENU = JSON.parse(JSON.stringify(SY_MENU_SEED));
  try { localStorage.setItem(SY_MENU_KEY, JSON.stringify(SY_MENU)); } catch (e) {}
}
/* 一次性补种：菜单存在 localStorage 里，后来新增的节点（如「采购定价」「价格审批」）不会自动出现。
 * 缺了它们，角色就"勾不到"这两个页面 —— 而入库/价格的审核人正是靠菜单权限指定的，等于审核人无从配置。
 * 按 id 补齐缺失节点，标记位保证只补一次（用户之后手动删掉不会再长回来）。 */
var SY_MENU_SEED_FLAG = 'tcm_sys_menu_seed_v2';
function syMenuSeedMerge() {
  var done = false;
  try { done = localStorage.getItem(SY_MENU_SEED_FLAG) === '1'; } catch (e) {}
  if (done) return;
  var added = 0;
  SY_MENU_SEED.forEach(function (seedTop) {
    var top = null;
    for (var i = 0; i < SY_MENU.length; i++) if (SY_MENU[i].id === seedTop.id) top = SY_MENU[i];
    if (!top) { SY_MENU.push(JSON.parse(JSON.stringify(seedTop))); added++; return; }
    if (!top.children) top.children = [];
    (seedTop.children || []).forEach(function (sc) {
      var has = false;
      for (var j = 0; j < top.children.length; j++) if (top.children[j].id === sc.id) has = true;
      if (!has) { top.children.push(JSON.parse(JSON.stringify(sc))); added++; }
    });
  });
  if (added) syMenuPersist();
  try { localStorage.setItem(SY_MENU_SEED_FLAG, '1'); } catch (e) {}
}
