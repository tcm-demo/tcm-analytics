...
// 切换门店类型时（仅新增态）重填改价控制默认；编辑态不覆盖已保存配置
function smApplyTypeDefaults() {
  var t = document.getElementById('sfStoreType');
  if (!t || STORE_EDIT_ID !== null) return;
  var def = smDefaultPriceModes(t.value);
  var c = document.querySelector('input[name="cashierPriceMode"][value="' + def.cashier + '"]');
  var cd = document.querySelector('input[name="codingPriceMode"][value="' + def.coding + '"]');
  if (c) c.checked = true;
...
function initStoreForm() {
  ...
  // 改价控制：编辑用已存值，新增用类型默认值
  var priceDef = smDefaultPriceModes(defType);
  var cashierMode = (d && d.cashierPriceMode) ? d.cashierPriceMode : priceDef.cashier;
  var codingMode = (d && d.codingPriceMode) ? d.codingPriceMode : priceDef.coding;
  function sfPriceRadios(name, val) {
    var opts = [['forbid', '禁止改价'], ['temp', '临时改价'], ['permanent', '永久改价']];
    return opts.map(function(o){
      var chk = (val === o[0]) ? ' checked' : '';
      // 固定列宽 90px，避免中文字符宽度差异导致两行三列错位
      return '<label style="display:inline-flex;align-items:center;cursor:pointer;width:90px;margin-right:14px;font-size:12px;color:#303133"><input type="radio" name="' + name + '" value="' + o[0] + '"' + chk + ' style="margin-right:6px;flex-shrink:0"> ' + o[1] + '</label>';
    }).join('');
  }
  ...
  el.innerHTML =
    '<div class="gf-page">' +
      ...
      '</div>' +
      // 表单主体（基础信息全宽顶栏 + 左右两列：左=门店+其他，右=法人+证照+功能）
      '<div class="gf-body">' +
        ...
        '<div class="gf-section" style="margin-bottom:28px">' +
          ...
          '<div>' +
            ...
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              ...
              '<select class="gf-input" id="sfStoreType" onchange="smApplyTypeDefaults()" style="flex:1;height:32px;padding:0 10px;border:1px solid #e8e8e8;border-radius:4px;font-size:12px;outline:none;box-sizing:border-box;background:#fff">' +
                typeOpts +
              '</select>' +
            '</div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
          ...
          '</div>' +
        '</div>' +
        '<!-- 功能配置（库存开关 + 改价控制，独立节；三个 row 完全同构：label 120px 右对齐 + div 控件容器，与其它节的 gf-row 风格一致） -->' +
        '<div class="gf-section" style="margin-bottom:28px">' +
          '<div class="gf-section-hd">功能配置</div>' +
          '<div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              ...
              '<div style="display:flex;align-items:center"><label style="display:inline-flex;align-items:center;cursor:pointer"><input type="checkbox" id="sfInventorySwitch" '+(d&&d.inventorySwitch===1?'checked':'')+' style="margin-right:6px">开启</label></div>' +
            '</div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              '<label class="gf-label" style="width:120px;text-align:right;padding-right:12px;color:#606266;white-space:nowrap;font-size:12px;flex-shrink:0">收银改价</label>' +
              '<div style="display:flex;flex-wrap:wrap;align-items:center">' + sfPriceRadios('cashierPriceMode', cashierMode) + '</div>' +
            '</div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              '<label class="gf-label" style="width:120px;text-align:right;padding-right:12px;color:#606266;white-space:nowrap;font-size:12px;flex-shrink:0">打码改价</label>' +
              '<div style="display:flex;flex-wrap:wrap;align-items:center">' + sfPriceRadios('codingPriceMode', codingMode) + '</div>' +
            '</div>' +
          '</div>' +
        '</div>' +
        '</div>' +   // closes left col
...
function smSaveForm() {
  ...
  var cashierMode = (document.querySelector('input[name="cashierPriceMode"]:checked') || {}).value || 'permanent';
  var codingMode = (document.querySelector('input[name="codingPriceMode"]:checked') || {}).value || 'permanent';
...
// 商品列表「活动价」单元格：调用打折活动页的 saCalcPrice（含会员价取低）
function glSaleActivityCell(r) {
  ...
  if (!res || res.source === 'none') return '<span style="color:#c0c4cc;font-size:12px">-</span>';
  var color = (res.source === 'member') ? '#7b1fa2' : '#f56c6c';
  var tag = (res.source === 'member') ? '会员' : (res.detail && res.detail.indexOf('一口价') === 0) ? '一口价' : '活动';
