...
function navigateTo(pid) {
  var q = '';
  var i = pid.indexOf('?');
  if (i >= 0) { q = pid.substring(i); pid = pid.substring(0, i); }
  window.location.href = pid + '.html' + q;
}
...
// ===== 首次打开引导：未看过「应用场景一览」时先跳转到该页 =====
// 落地规则：除 scenario 页本身外，任何页面首次加载（localStorage 无标记）都
// 先 replace 到 scenario.html?first=1&back=<本页>，由该页做 20s 展示/跳过；
// 看过（跳过或倒计时结束）后写入标记，此后一切页面直达，不再引导。
(function () {
  var KEY = 'tcm_firstview_scenario_v1';
  function fvSeen() {
    try { return !!window.localStorage.getItem(KEY); } catch (e) { return false; }
  }
  if (CURRENT_PAGE === 'scenario') return;   // 引导页自身不拦截
  if (urlParam('first') === '1') return;     // 引导返回残留参数，避免死循环
  if (fvSeen()) return;                      // 已看过，直达
  var back = CURRENT_PAGE + '.html' + (window.location.search || '');
  window.location.replace('scenario.html?first=1&back=' + encodeURIComponent(back));
})();
...
function initMultiPage(pid) {
  ...
  // Hide scope bar for pages that don't need it (NO_SCOPE_PAGES 由 shared/header.js 统一声明)
  var hideScope = NO_SCOPE_PAGES.indexOf(pid) >= 0;
  ...
  // Init page is delegated to activatePage() above (runs synchronously)
}
...
var TICKER_DATA = [
  { date: '09/08', type: 'new', title: '定价策略', page: 'dynamic-pricing', status: 'done' },
  { date: '09/08', type: 'new', title: '库存盘点', page: 'inv-check', status: 'done' },
];
...
// ===== NAVIGATION =====
// 注意：pageTitles / PAGE_GROUPS 已提升到 header.js（早于 layout.js 加载），供页头首屏即渲染正确标题；此处不再重复声明。
...
function activatePage(id) {
  ...
  var scopeSep = document.querySelectorAll('.scope-bar .scope-sep');
  var scopeSpacer = document.querySelectorAll('.scope-bar .scope-spacer');
  // NO_SCOPE_PAGES（由 shared/header.js 统一声明）的页面不显示 scope-bar / 日期 / 企业&门店选择器
  var hideScope = NO_SCOPE_PAGES.indexOf(id) >= 0;
  if (scopeBar) {
...
function initPage(id) {
  const fns = {
    ...
    'sale-activity': function() { if (typeof initSaleActivity === 'function') initSaleActivity(); },
...
function closeGroupForm() {
  showToast(GROUP_EDIT_ID ? '保存成功' : '新增成功');
  setTimeout(function(){ navigateTo('group-manage'); }, 500);
}

function closeStoreForm() {
  showToast(STORE_EDIT_ID !== null ? '保存成功' : '新增成功');
  setTimeout(function(){ navigateTo('store-manage'); }, 500);
}
...
function gmCancelForm() {
  navigateTo('group-manage');
}

function showGroupModal(editId) {
  navigateTo('group-form' + (editId ? '?edit=' + encodeURIComponent(editId) : ''));
}
...
function initGroupForm() {
  ...
  GROUP_EDIT_ID = urlParam('edit') || null;
  var isEdit = !!GROUP_EDIT_ID;
  var titleText = isEdit ? '编辑企业' : '新增企业';
  var d = null;
  if (isEdit) {
    d = GROUP_DATA.find(function(r){return String(r.companyId) === String(GROUP_EDIT_ID);});
  }
  ...
  el.innerHTML =
    '<div class="gf-page">' +
      ...
      '</div>' +
      // ===== 表单主体（左右两列：左=企业信息，右=法人信息+联系信息）=====
      '<div class="gf-body">' +
        '<div class="gf-body--split">' +
          '<div class="gf-col">' +
          ...
          '<div class="gf-row">' +
            ...
            '<div class="gf-cell">' +
              '<div style="display:flex;align-items:center;gap:14px;height:32px">' +
                '<label style="display:inline-flex;align-items:center;cursor:pointer;width:100px;font-size:14px;color:#303133"><input type="radio" name="gType" value="0" '+(v('type')===1?'':'checked')+' onchange="gmToggleType()" style="margin-right:6px;flex-shrink:0"> 实体企业</label>' +
                '<label style="display:inline-flex;align-items:center;cursor:pointer;width:100px;font-size:14px;color:#303133"><input type="radio" name="gType" value="1" '+(v('type')===1?'checked':'')+' onchange="gmToggleType()" style="margin-right:6px;flex-shrink:0"> 非实体组织</label>' +
              '</div>' +
            '</div>' +
          '</div>' +
          // 品牌Logo (label + 上传)
...
// ===== 门店类型 → 改价控制默认值 =====
// 菜店默认：收银改价-永久改价、打码改价-永久改价
// 商超默认：收银改价-禁止改价、打码改价-临时改价
function smDefaultPriceModes(type) {
  if (type === '商超') return { cashier: 'forbid', coding: 'temp' };
  return { cashier: 'permanent', coding: 'permanent' }; // 菜店 默认
}

function initStoreForm() {
  var el = document.getElementById('storeFormContent');
  if (!el) { setTimeout(initStoreForm, 80); return; }
  STORE_EDIT_ID = urlParam('edit') || null;
  var isEdit = STORE_EDIT_ID !== null;
  var title = document.getElementById('storeFormTitle');
  if (title) title.textContent = isEdit ? '编辑门店' : '新增门店';

  var d = isEdit ? (function(){ var r=null; STORE_DATA.forEach(function(s){if(String(s.shopId)===String(STORE_EDIT_ID))r=s;}); return r; })() : null;
  ...
  // 新增可选可切换；编辑锁定不可改（disabled 仍可被 smSaveForm 读到当前值）
  var companyEmpty = isEdit ? '' : '<option value="">请选择企业</option>';
  var companyAttrs = isEdit ? ' disabled' : '';
  var companyBg = isEdit ? '#f5f7fa;color:#909399;cursor:not-allowed' : '#fff';
  // 门店类型（驱动改价控制默认值）
  var defType = (d && d.storeType) ? d.storeType : '菜店';
  var typeOpts = ['菜店', '商超'].map(function(t){
    return '<option value="' + t + '"' + (defType === t ? ' selected' : '') + '>' + t + '</option>';
  }).join('');
  ...
  el.innerHTML =
    '<div class="gf-page">' +
      ...
      '</div>' +
      // 表单主体（基础信息全宽顶栏 + 左右两列：左=门店信息，右=法人+其他证照）
      '<div class="gf-body">' +

        '<!-- 基础信息（全宽顶栏：所属企业 + 联系人 + 联系电话 并排） -->' +
        '<div class="gf-section" style="margin-bottom:28px">' +
          '<div class="gf-section-hd">基础信息</div>' +
          '<div style="display:flex;gap:32px;align-items:flex-start;flex-wrap:wrap">' +
            '<div style="flex:1 1 240px;min-width:0">' +
              '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
                ...
                '<select class="gf-input" id="sfCompanyId"' + companyAttrs + ' style="flex:1;height:32px;padding:0 10px;border:1px solid #e8e8e8;border-radius:4px;font-size:12px;outline:none;box-sizing:border-box;background:' + companyBg + '">' +
                  companyEmpty + companyOpts +
                '</select>' +
              '</div>' +
            '</div>' +
            '<div style="flex:1 1 240px;min-width:0">' +
        ...
        '<div class="gf-section" style="margin-bottom:28px">' +
          ...
          '<div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              ...
              '<div class="sf-upload" id="sfDoorUpload" style="width:110px;height:110px;border:1px dashed #dcdfe6;border-radius:4px;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;position:relative;overflow:hidden" onclick="smTriggerUpload(\'shopFrontFileId\',\'sfDoorPreview\',this)">' +
            ...
            '</div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              '<label class="gf-label" style="width:120px;text-align:right;padding-right:12px;color:#606266;white-space:nowrap;font-size:12px;flex-shrink:0">门店类型</label>' +
              '<select class="gf-input" id="sfStoreType" style="flex:1;height:32px;padding:0 10px;border:1px solid #e8e8e8;border-radius:4px;font-size:12px;outline:none;box-sizing:border-box;background:#fff">' +
                typeOpts +
              '</select>' +
            '</div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              ...
              '<div class="sf-upload" id="sfLicenseUpload" style="width:110px;height:110px;border:1px dashed #dcdfe6;border-radius:4px;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;position:relative;overflow:hidden" onclick="smTriggerUpload(\'businessLicenseFileId\',\'sfLicensePreview\',this)">' +
            ...
            '</div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              '<label class="gf-label" style="width:120px;text-align:right;padding-right:12px;color:#606266;white-space:nowrap;font-size:12px;flex-shrink:0"><span style="color:#f56c6c;margin-right:2px">*</span>信用代码</label>' +
              '<input class="gf-input" id="sfLicenseNo" value="'+(d?d.businessLicenseNo:'')+'" placeholder="请输入统一社会信用代码">' +
            '</div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              '<label class="gf-label" style="width:120px;text-align:right;padding-right:12px;color:#606266;white-space:nowrap;font-size:12px;flex-shrink:0"><span style="color:#f56c6c;margin-right:2px">*</span>注册地址</label>' +
              '<input class="gf-input" id="sfBizRegAddress" value="'+(d?d.businessRegistrationAddress:'')+'" placeholder="请输入注册地址">' +
            '</div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              '<label class="gf-label" style="width:120px;text-align:right;padding-right:12px;color:#606266;white-space:nowrap;font-size:12px;flex-shrink:0"><span style="color:#f56c6c;margin-right:2px">*</span>营业执照有效期</label>' +
          ...
          '<div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              ...
              '<div style="display:flex;gap:12px">' +
                '<div class="sf-upload" id="sfCardFrontUpload" style="width:110px;height:110px;border:1px dashed #dcdfe6;border-radius:4px;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;position:relative;overflow:hidden" onclick="smTriggerUpload(\'identityCardFrontFileId\',\'sfCardFrontPreview\',this)">' +
                ...
                '</div>' +
                '<div class="sf-upload" id="sfCardBackUpload" style="width:110px;height:110px;border:1px dashed #dcdfe6;border-radius:4px;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;position:relative;overflow:hidden" onclick="smTriggerUpload(\'identityCardBackFileId\',\'sfCardBackPreview\',this)">' +
        ...
        '<div class="gf-section" style="margin-bottom:28px">' +
          ...
          '<div>' +
            '<div class="gf-row" style="display:flex;align-items:center;margin-bottom:18px">' +
              ...
              '<div class="sf-upload" id="sfPermitUpload" style="width:110px;height:110px;border:1px dashed #dcdfe6;border-radius:4px;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;position:relative;overflow:hidden" onclick="smTriggerUpload(\'licenseFileId\',\'sfPermitPreview\',this)">' +
...
function smSaveForm() {
  ...
  var storeType = document.getElementById('sfStoreType').value;
  // 价格管控不在此表单配置（已移至「门店管理 → 管控配置」）：
  // 新增按门店类型给默认值；编辑沿用原值，不做任何覆盖
  var priceDef = smDefaultPriceModes(storeType);
  var cashierMode = (isEdit && d && d.cashierPriceMode) ? d.cashierPriceMode : priceDef.cashier;
  var codingMode = (isEdit && d && d.codingPriceMode) ? d.codingPriceMode : priceDef.coding;
  var invSwitch = (isEdit && d) ? (d.inventorySwitch === 1 ? 1 : 0) : 0;
  ...
  var data = {
    companyId: companyId,
    storeType: storeType,
    cashierPriceMode: cashierMode,
    codingPriceMode: codingMode,
  ...
  var _smSavedMsg = (STORE_EDIT_ID ? '保存成功' : '新增成功');
  tcmPersistGroupStore();
  STORE_EDIT_ID = null;
  showToast(_smSavedMsg);
  setTimeout(function(){ navigateTo('store-manage'); }, 500);
}
...
function smRenderTable() {
  ...
  tbody.innerHTML = pageData.length ? pageData.map(function(r, idx){
    ...
    var ccBadge = '';
    if (typeof spCcGet === 'function') {
      var cc = spCcGet(r.shopId), fn = smFnGet(r.shopId);
      // 库存开关是进价管控的前提：不入库就没有送货价可校验，此时「统一定价」不执行，
      // 标出来是误导，改标「未开库存」说明原因（不静默省略，否则看不出是没配还是没生效）
      var invOn = fn.inventorySwitch === 1;
      ccBadge = '<div style="font-size:11px;color:' + (!invOn ? '#8a93a3' : (cc.uniformPrice ? '#67c23a' : '#8a93a3')) + '"'
        + (invOn ? '' : ' title="本店未开启库存管理，进价管控（统一定价）不生效"') + '>'
        + (invOn ? (cc.uniformPrice ? '统一定价' : '自由价') : '未开库存') + ' · 收银' + smFnModeText(fn.cashierPriceMode) + '</div>';
    }
    return '<tr data-shop-id="'+r.shopId+'" onclick="smSelectRow(this)" style="cursor:pointer">' +
      ...
      '<td>'+statusHtml+'</td>' +
      '<td><a style="color:#1677ff;cursor:pointer" onclick="event.stopPropagation();smOpenControlConfig(\''+r.shopId+'\')">管控配置</a><div>'+ccBadge+'</div></td>' +
    '</tr>';
  }).join('') : '<tr><td colspan="9" style="text-align:center;color:#999;padding:30px">暂无数据</td></tr>';
...
function smOpenAdd() {
  navigateTo('store-form');
}

function gmOpenEdit(id) {
  navigateTo('group-form?edit=' + encodeURIComponent(id));
}

/* ===== 管控配置（门店级）：一个入口，两类管控 =====
 * 位置：门店管理 → 每行「管控配置」入口。配置是门店级的，同企业下直营店可以严、加盟店松。
 *   进价管控 = 成本控制：供应商送货价怎么卡，存 tcm_cost_control_v1，改动留痕。
 *   售价与功能 = 库存开关 / 收银改价 / 打码改价：门店自己能不能动售价，存门店对象字段。
 *   两者是同一「价格管控」的两端，合成一个入口，避免操作列每加一类管控就多一个链接。
 *   门店建档表单只登记资质资料，不做功能配置——三项统一在本弹窗配置（2026-09-09 起）。
 * 价格提醒不可关闭：开了统一定价就必须提醒（录入/详情/列表/台账），与处理规则正交。
 */
var SM_CC_DRAFT = null, SM_FN_DRAFT = null, SM_CFG_STORE = null, SM_CFG_TAB = 'func';
/* 弹窗高度按内容自适应——上一版锁高 70vh/600px 看似让两页整齐，实际把内容撑不满的页"注水"成大段空白，
 * 反而需要滚动才能看完末尾的 spec note。正确做法是：高度 = 内容自然高度，CSS 兜底 max-height:85vh，
 * 只在内容真的超出视口时才滚动。margin-top:auto 也一并取消（人造空白是问题的根源，不是解法）。 */
var SM_CFG_WIDTH = 'min(780px,94vw)';
function smCcModal(opt) {
  // 弹窗已存在时只换内容，不销毁重建：否则切标签页会先塌陷再撑开 + 闪一帧，观感就是"高度在跳"
  var exist = document.getElementById('smCcModal');
  if (exist && !opt.rebuild) {
    var eb = exist.querySelector('.ic-modal-body');
    if (eb) { eb.innerHTML = opt.body; eb.scrollTop = 0; }
    var et = exist.querySelector('.ic-modal-header span'); if (et) et.textContent = opt.title || '';
    var eok = exist.querySelector('.ic-modal-footer .btn-primary'); if (eok && opt.onOk) eok.setAttribute('onclick', opt.onOk);
    return;
  }
  smCcCloseModal();
  var bd = document.createElement('div'); bd.className = 'ic-modal-backdrop'; bd.id = 'smCcBackdrop';
  bd.onclick = function (e) { if (e.target === this) smCcCloseModal(); };
  var md = document.createElement('div'); md.className = 'ic-modal'; md.id = 'smCcModal';
  // 取消 max-height 放宽：恢复 CSS 默认的 85vh 兜底，超出才滚
  md.style.cssText = 'width:' + (opt.width || SM_CFG_WIDTH) + ';';
  // 内容区不加 fixed-h（高度自适应，无人为锁高）
  md.innerHTML = '<div class="ic-modal-header"><span>' + (opt.title || '') + '</span><button class="ic-modal-close" onclick="smCcCloseModal()">✕</button></div>'
    + '<div class="ic-modal-body">' + opt.body + '</div>'
    + '<div class="ic-modal-footer">'
    + (opt.footLeft || '')
    + '<button class="btn-secondary" onclick="smCcCloseModal()">取消</button>'
    + '<button class="btn-primary" onclick="' + opt.onOk + '">保存</button></div>';
  document.body.appendChild(bd); document.body.appendChild(md);
}
function smCcCloseModal() {
  var b = document.getElementById('smCcBackdrop'); if (b) b.remove();
  var m = document.getElementById('smCcModal'); if (m) m.remove();
}
function smCfgCostBody(cc, invOn) {
  function sel(field, opts) {
    return '<select style="width:240px;height:30px;padding:0 8px;border:1px solid #e8e8e8;border-radius:4px;font-size:12px;background:#fff;outline:none" onchange="SM_CC_DRAFT.' + field + '=this.value">' +
      opts.map(function (o) { return '<option value="' + o[0] + '"' + (cc[field] === o[0] ? ' selected' : '') + '>' + o[1] + '</option>'; }).join('') + '</select>';
  }
  function sw(field, label) {
    var on = !!cc[field];
    return '<label style="display:inline-flex;align-items:center;gap:6px;cursor:pointer;font-size:12px">' +
      '<input type="checkbox" ' + (on ? 'checked' : '') + ' onchange="SM_CC_DRAFT.' + field + '=this.checked" style="width:14px;height:14px">' + label + '</label>';
  }
  var core = '<div style="border:1px solid #e9eef7;border-radius:4px;padding:12px;margin-bottom:10px">' +
      '<div style="font-size:12px;font-weight:600;color:#1a2233;margin-bottom:10px">' + sw('uniformPrice', '统一定价') + ' <span style="color:#8a93a3;font-weight:400;margin-left:8px">开启后，入库价与本店定价不一致时按下列规则处理</span></div>' +
      '<div style="display:grid;grid-template-columns:110px 1fr;gap:10px 12px;align-items:center;font-size:12px;padding-left:22px">' +
        '<span style="color:#5b6472">未定价商品</span>' + sel('noPriceAction', [['forbid', '禁止入库'], ['audit', '审核后入库'], ['allow', '允许入库并提醒']]) +
        '<span style="color:#5b6472">高价处理规则</span>' + sel('highAction', [['forbid', '禁止入库'], ['audit', '审核后入库']]) +
        '<span style="color:#5b6472">低价处理规则</span>' + sel('lowAction', [['entry_only', '仅入库(不修改采购报价)'], ['forbid', '禁止入库'], ['audit', '审核后入库']]) +
      '</div>' +
    '</div>' +
    '<div style="border:1px solid #e9eef7;border-radius:4px;padding:12px;margin-bottom:10px">' +
      '<div style="font-size:12px;font-weight:600;color:#1a2233">' + sw('entryAudit', '入库审核') +
        ' <span style="color:#8a93a3;font-weight:400;margin-left:8px">所有入库单均需审核确认后才入库（与价格无关）；审核人在「价格审批 → 审核设置」中指定（demo 不校验权限，仅记录审核人）</span></div>' +
    '</div>' +
    '<div style="display:grid;grid-template-columns:110px 1fr;gap:10px 12px;align-items:center;font-size:12px">' +
      '<span style="color:#5b6472">单行容差</span><span><input type="number" step="0.01" value="' + (cc.tolAmount || 0.05) + '" oninput="SM_CC_DRAFT.tolAmount=parseFloat(this.value)||0" style="width:80px;height:30px;padding:0 8px;border:1px solid #e8e8e8;border-radius:4px;font-size:12px"> 元 或 ' +
      '<input type="number" step="0.1" value="' + (cc.tolPct || 1) + '" oninput="SM_CC_DRAFT.tolPct=parseFloat(this.value)||0" style="width:70px;height:30px;padding:0 8px;border:1px solid #e8e8e8;border-radius:4px;font-size:12px"> %，取两者较大值</span>' +
    '</div>' +
    smCfgNote('spec', '<b style="color:#1a2233">提醒不可关闭</b>：统一定价开启后，只要价格与定价不一致就必定提醒（录入时 / 单据详情 / 列表波动列 / 价格波动台账）。<br>上面配的是「怎么处理」，不是「要不要提醒」——便宜了也要让采购经理知道，否则定价体系会乱。<br>本组规则的执行对象是<b>入库单</b>：库存开关关闭时本店不做入库，整块不生效（切到「售价与功能」可开启）。');
  if (!invOn) {
    return '<div style="margin-bottom:12px;padding:10px 12px;background:#fff7e6;border:1px solid #ffd591;border-radius:4px;font-size:12px;color:#8a5a00;line-height:20px">' +
      '<b>库存开关已关闭</b>：本店不做入库与盘点，下面这些规则没有执行对象，保存后也不会生效（已配内容保留，重新开启库存后照原值执行）。' +
      '<a style="color:#005cf5;cursor:pointer;margin-left:8px" onclick="smCfgEnableInv()">开启库存开关</a>' +
      '</div>' +
      '<div style="opacity:.45;pointer-events:none;user-select:none">' + core + '</div>';
  }
  var cnt = smCfgPriceCount(SM_CFG_STORE);
  if (cnt === 0 && cc.uniformPrice && cc.noPriceAction === 'forbid') {
    return '<div style="margin-bottom:12px;padding:10px 12px;background:#fef0f0;border:1px solid #fbc4c4;border-radius:4px;font-size:12px;color:#a32d2d;line-height:20px">' +
      '<b>本店尚未维护采购定价</b>，而「未定价商品」配的是「禁止入库」——保存后本店<b>所有商品都将无法入库</b>。' +
      '请先到 <a style="color:#005cf5;cursor:pointer" onclick="smCcCloseModal();navigateTo(\'supply-price\')">采购定价</a> 维护价格，或把下面的「未定价商品」改为「允许入库并提醒」。</div>' + core;
  }
  // 正常态不显示「已生效采购定价 N 条」：这是配置页，不是定价看板，条数在采购定价页看。
  // 只在会出事的组合下告警（零定价 + 未定价禁止入库 = 全店停摆）。
  return core;
}
// 库存开关是进价管控的前提：关了就没有入库单，价格校验无从执行
function smCfgEnableInv() {
  if (SM_FN_DRAFT) SM_FN_DRAFT.inventorySwitch = 1;
  smCfgRender();
}
// 统一定价的第二个前提：本店得有定价数据，否则「未定价禁止入库」= 全店停摆
function smCfgPriceCount(storeId) {
  var n = 0;
  if (typeof SP_PRICES !== 'undefined') {
    for (var i = 0; i < SP_PRICES.length; i++) {
      if (SP_PRICES[i].storeId === storeId && String(SP_PRICES[i].status) === '20') n++;
    }
  }
  return n;
}
...
/* ===== 售价与功能（门店属性）=====
 * 库存开关 / 收银改价 / 打码改价 是门店对象上的字段，随门店资料下发秤端与收银端。
 * 默认值按门店类型给（见 smDefaultPriceModes）；这里只是运营期的快捷改法。
 */
function smFnGet(storeId) {
  var s = null;
  for (var i = 0; i < STORE_DATA.length; i++) if (String(STORE_DATA[i].shopId) === String(storeId)) s = STORE_DATA[i];
  var def = smDefaultPriceModes((s && s.storeType) || '菜店');
  return {
    inventorySwitch: (s && s.inventorySwitch === 1) ? 1 : 0,
    cashierPriceMode: (s && s.cashierPriceMode) || def.cashier,
    codingPriceMode: (s && s.codingPriceMode) || def.coding
  };
}
/* 说明条：写给人看的话必须标明身份，避免把"没做完"当"产品逻辑"讲给客户。
 * type='spec' 产品说明 —— 对客户/实施讲的业务逻辑，正式版原样保留
 * type='dev'  开发提醒 —— demo 未实现、需后续补做的部分，正式版应删除或兑现
 */
function smCfgNote(type, html) {
  var dev = type === 'dev';
  return '<div style="margin-top:12px;padding:8px 12px;border-radius:4px;font-size:12px;line-height:20px;' +
    (dev ? 'background:#fff7e6;border:1px solid #ffd591;color:#8a5a00' : 'background:#f7f9fc;border:1px solid #e9eef7;color:#5b6472') + '">' +
    '<span style="display:inline-block;margin-right:6px;padding:1px 6px;border-radius:3px;font-size:11px;' +
    (dev ? 'background:#ffe7ba;color:#a35c00' : 'background:#e8eefb;color:#185fa5') + '">' + (dev ? '开发提醒' : '产品说明') + '</span>' +
    html + '</div>';
}
function smFnModeText(m) {
  return ({ forbid: '禁改', temp: '临时', permanent: '永久' })[m] || '—';
}
function smOpenControlConfig(storeId, tab) {
  if (typeof spCcGet !== 'function') { alert('定价模块未加载'); return; }
  SM_CFG_STORE = storeId;
  SM_CFG_TAB = tab || 'func';
  SM_CC_DRAFT = JSON.parse(JSON.stringify(spCcGet(storeId)));
  SM_FN_DRAFT = smFnGet(storeId);
  smCfgRender();
}
function smOpenCostControl(storeId) { smOpenControlConfig(storeId, 'cost'); }
function smCfgSwitch(tab) { SM_CFG_TAB = tab; smCfgRender(); }
function smCfgRender() {
  var storeId = SM_CFG_STORE, s = null;
  for (var i = 0; i < STORE_DATA.length; i++) if (String(STORE_DATA[i].shopId) === String(storeId)) s = STORE_DATA[i];
  var name = (s && (s.shopShortName || s.shopName)) || storeId;
  var tabs = [['func', '售价与功能'], ['cost', '进价管控']];
  var tabHtml = tabs.map(function (t) {
    var on = SM_CFG_TAB === t[0];
    return '<span onclick="smCfgSwitch(\'' + t[0] + '\')" style="cursor:pointer;font-size:13px;margin-right:24px;padding-bottom:7px;display:inline-block;color:' +
      (on ? '#005cf5' : '#5b6472') + ';border-bottom:2px solid ' + (on ? '#005cf5' : 'transparent') + '">' + t[1] + '</span>';
  }).join('');
  var body =
    '<div style="font-size:12px;color:#5b6472;margin-bottom:12px">门店：<b style="color:#0b1019">' + name + '</b>　配置仅对本店生效，不影响其他门店</div>' +
    '<div style="border-bottom:1px solid #e9eef7;margin-bottom:14px">' + tabHtml + '</div>' +
    (SM_CFG_TAB === 'cost' ? smCfgCostBody(SM_CC_DRAFT, !!(SM_FN_DRAFT && SM_FN_DRAFT.inventorySwitch === 1)) : smCfgFuncBody());
  smCcModal({ title: '管控配置 · ' + name, body: body, onOk: 'smSaveControlConfig()' });
}
function smCfgFuncBody() {
  var f = SM_FN_DRAFT;
  function radios(field, val) {
    return [['forbid', '禁止改价'], ['temp', '临时改价'], ['permanent', '永久改价']].map(function (o) {
      return '<label style="display:inline-flex;align-items:center;cursor:pointer;width:96px;margin-right:8px;font-size:12px;color:#303133">' +
        '<input type="radio" name="fn_' + field + '" value="' + o[0] + '"' + (val === o[0] ? ' checked' : '') +
        ' onchange="SM_FN_DRAFT.' + field + '=this.value" style="margin-right:6px"> ' + o[1] + '</label>';
    }).join('');
  }
  function sw(field, label, text) {
    var on = (f[field] === 1 || f[field] === '1' || f[field] === true);
    return '<label style="display:inline-flex;align-items:center;gap:6px;cursor:pointer;font-size:12px">' +
      '<input type="checkbox" ' + (on ? 'checked' : '') + ' onchange="SM_FN_DRAFT.' + field + '=this.checked?1:0" style="width:14px;height:14px">' + label + '</label>' +
      '<span style="color:#8a93a3;font-weight:400;margin-left:8px">' + text + '</span>';
  }
  // 按功能分栏，与「进价管控」同构：库存归库存、改价归改价，两件事不混在一张卡里
  return '<div style="border:1px solid #e9eef7;border-radius:4px;padding:12px;margin-bottom:10px">' +
      '<div style="font-size:12px;font-weight:600;color:#1a2233;margin-bottom:8px">' +
        sw('inventorySwitch', '库存开关', '关闭后本店不做入库与盘点，「进价管控」整块不生效') + '</div>' +
      '<div style="font-size:12px;color:#5b6472;line-height:20px;padding-left:22px">' +
        '库存开关是「进价管控」的<b>前提</b>：不入库就没有送货价，价格校验无从执行。</div>' +
    '</div>' +
    '<div style="border:1px solid #e9eef7;border-radius:4px;padding:12px">' +
      '<div style="font-size:12px;font-weight:600;color:#1a2233;margin-bottom:10px">改价权限' +
        '<span style="color:#8a93a3;font-weight:400;margin-left:8px">门店自己能不能动售价，按场景分开配</span></div>' +
      '<div style="display:grid;grid-template-columns:110px 1fr;gap:14px 12px;align-items:center;font-size:12px">' +
        '<span style="color:#5b6472">收银改价</span><div style="display:flex;flex-wrap:wrap;align-items:center">' + radios('cashierPriceMode', f.cashierPriceMode) + '</div>' +
        '<span style="color:#5b6472">打码改价</span><div style="display:flex;flex-wrap:wrap;align-items:center">' + radios('codingPriceMode', f.codingPriceMode) + '</div>' +
      '</div>' +
      '<div style="margin-top:10px;font-size:12px;color:#5b6472;line-height:20px">' +
        '收银改价＝<b>结账时</b>能不能改；打码改价＝<b>打价签时</b>能不能改。<br>' +
        '「临时」只对当次有效，「永久」会写回商品售价——一般收银端收紧、打码端放宽。</div>' +
    '</div>' +
  // 说明位置：开发提醒在前，产品说明在后——风险提示先看到，正式说明兜底解释
  smCfgNote('dev', '<b>库存开关目前只是存档字段</b>：关闭它不会真的阻止创建入库单，只在本弹窗内让「进价管控」置灰。正式版需在入库入口按此开关拦截（先要有「当前门店上下文」，demo 暂无）。<br>本条为演示/开发提醒，正式版应删除或兑现。') +
  smCfgNote('spec', '<b style="color:#1a2233">这里管的是售价端</b>，和「进价管控」两件事：进价管控卡<b>供应商送货价</b>，这里管<b>门店自己能不能动售价</b>。<br>' +
    '门店建档只登记资质资料，这三项统一在本弹窗配置。默认值按门店类型给（菜店：收银/打码均可永久改价；商超：收银禁改、打码临时改），新建门店默认库存开关为<b>关</b>，需入库/盘点的门店在此开启。');
}
function smSaveControlConfig() {
  if (!SM_CFG_STORE) return;
  var storeId = SM_CFG_STORE;
  var who = (typeof CURRENT_USER_NAME !== 'undefined' && CURRENT_USER_NAME) || '当前用户';
  var changed = 0;
  if (SM_CC_DRAFT) {
    var cc = spCcGet(storeId);
    // auditor 已废弃：审核人改由角色权限决定（2026-09-08 用户定），不再按门店指定个人。
    // 字段仍留在 SP_CC_FIELDS / 历史数据里，只为让旧留痕记录还能显示中文名，不再写入。
    ['uniformPrice', 'noPriceAction', 'highAction', 'lowAction', 'entryAudit', 'tolAmount', 'tolPct'].forEach(function (f) {
      var ov = cc[f], nv = SM_CC_DRAFT[f];
      if (String(ov) !== String(nv)) {
        cc[f] = nv;
        if (typeof spCcLog === 'function') spCcLog(storeId, f, ov, nv, who);
        changed++;
      }
    });
    if (changed) { cc.updatedBy = who; cc.updatedAt = new Date().toLocaleString('zh-CN'); }
    spCcPersist();
  }
  if (SM_FN_DRAFT) {
    var old = smFnGet(storeId), s = null;
    for (var i = 0; i < STORE_DATA.length; i++) if (String(STORE_DATA[i].shopId) === String(storeId)) s = STORE_DATA[i];
    if (s) {
      ['inventorySwitch', 'cashierPriceMode', 'codingPriceMode'].forEach(function (f) {
        if (String(old[f]) !== String(SM_FN_DRAFT[f])) {
          if (typeof spCcLog === 'function') spCcLog(storeId, f, old[f], SM_FN_DRAFT[f], who);
          s[f] = SM_FN_DRAFT[f];
          changed++;
        }
      });
  ...
  smCcCloseModal();
  if (typeof smRenderTable === 'function') smRenderTable();
  if (changed) { if (typeof showToast === 'function') showToast('已保存（改动已留痕）'); else alert('已保存（改动已留痕）'); }
}
function smOpenEdit(id) {
  navigateTo('store-form?edit=' + encodeURIComponent(id));
}

function smBatchEdit() {
  var ids = smGetSelectedIds();
  if (ids.length !== 1) { alert('请选择且仅选择一条记录进行编辑'); return; }
  navigateTo('store-form?edit=' + encodeURIComponent(ids[0]));
}
...
function shRenderTable() {
  ...
  thead.innerHTML = '<th>序号</th><th>日期</th><th>门店</th><th>人员</th>' +
    ...
    '<th>现金收款</th><th>线上收款</th><th>收款合计</th>' +
    '<th>应缴现金</th><th>实缴金额</th><th>长短款</th>' +
    '<th>订单笔数</th><th>备注</th><th>交班终端</th>';
  var html = '';
  if (page.length === 0) {
    html = '<tr><td colspan="16" style="text-align:center;color:#999;padding:40px">暂无交班记录</td></tr>';
  } else {
...
function pshRenderTable() {
  ...
  var html = '';
  if (page.length === 0) {
    html = '<tr><td colspan="10" style="text-align:center;color:#999;padding:40px">暂无交班记录</td></tr>';
  } else {
    for (var i = 0; i < page.length; i++) {
      var r = page[i];
      html += '<tr>' +
        ...
        '<td>' + _esc(r.endTime) + '</td>' +
        '<td>' + _esc(r.duration) + '</td>' +
        '<td style="font-weight:600;color:#005CF5">¥' + (r.actualCash || 0).toFixed(2) + '</td>' +
        '<td>' + r.orderCount + '笔</td>' +
        '<td style="max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:default" data-tip="' + _esc(r.note || '') + '">' + _esc(r.note || '—') + '</td>' +
        '<td>' + _esc(r.endTerminal) + '</td>' +
      '</tr>';
...
function pshRenderCards() {
  ...
  var m = { count: 0, actualCash: 0, orderCount: 0, totalMin: 0 };
  for (var i = 0; i < records.length; i++) {
    var r = records[i];
    m.count++;
  ...
  container.innerHTML =
    '<div style="display:flex;flex-wrap:wrap;gap:10px;padding:0 4px">' +
      ...
      '<div style="flex:1;min-width:120px;padding:14px 12px;background:#fff;border-radius:4px;border:1px solid #f0f0f0;text-align:center">' +
        '<div style="font-size:11px;color:#999;margin-bottom:4px">已上交现金</div>' +
        '<div style="font-size:18px;font-weight:800;color:#005CF5">¥' + m.actualCash.toFixed(2) + '</div>' +
      '</div>' +
    '</div>';
}
...
function pshSearch() {
  ...
  PSH_PAGE = 1; pshRenderTable(); pshRenderCards();
}
...
function initShiftHandover() {
  ...
  el.innerHTML =
    '<div style="flex-shrink:0;margin:0;padding:14px 24px;background:#fff;border-bottom:1px solid #f0f0f0;display:flex;align-items:center;gap:10px;flex-wrap:wrap">' +
      ...
      '<div style="flex:1"></div>' +
      '<button class="ic-btn" style="background:#5c6bc0;color:#fff;border-color:#5c6bc0" onclick="openShiftSimModal(SH_FILTER_STAFF !== \'all\' ? SH_FILTER_STAFF : CURRENT_STAFF, {admin:true})">模拟交班</button>' +
    '</div>' +
    // 人员总览卡片
...
moveGlobalPickers();
// 「更新时间」= 最近一次 git 提交时间。
// 数据来自 shared/build-info.js（由 .git/hooks/post-commit 调 gen_build_info.py 生成，不入仓库）。
// 该文件未生成时（克隆新机器还没跑过脚本）回退显示「更新时间 —」，不留假时间。
(function () {
  var el = document.getElementById('updateTime');
  if (!el) return;
  var b = window.BUILD_INFO;
  if (!b || !b.time) { el.textContent = '更新时间 —'; return; }
  el.textContent = '更新 ' + b.time + ' (' + b.commit + ')';
  if (b.message) el.title = b.commit + '　' + b.message;
})();
// ========== 全局 Tooltip（[data-tip]） ==========
(function() {
  var box = null;
...
var SH_SIM_REMARK_PRESETS = ['POS故障已报修', '网络异常已反馈', '零钱不足已申请', '系统卡顿已重启', '交接班正常', '其他异常'];
...
function simGenShiftData(staffOverride) {
  ...
  // 注：底钱（备用金）由门店线下自行管理，系统不再生成与记录该字段
  // 生成 mock 订单
...
// 交班弹框 DOM 懒创建：pages/*.html 均为壳页，拆分后不再内置 shiftSimOverlay，
// 首次打开时按需注入，避免无关页面残留冗余 DOM，也防止 overlay 为 null 导致点击无响应。
function simEnsureModal() {
  var overlay = document.getElementById('shiftSimOverlay');
  if (overlay) return overlay;
  overlay = document.createElement('div');
  overlay.id = 'shiftSimOverlay';
  ...
  overlay.addEventListener('click', function(e) { if (e.target === overlay) closeShiftSimModal(); });
  overlay.innerHTML =
    '<div id="shiftSimModal" style="background:#fff;border-radius:12px;width:90vw;max-width:1000px;max-height:90vh;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,0.3)">' +
      '<div style="flex-shrink:0;padding:16px 20px;border-bottom:1px solid #e0e0e0;display:flex;justify-content:space-between;align-items:center">' +
        '<span id="shiftSimTitle" style="font-size:16px;font-weight:700;color:#333">交班录入</span>' +
        '<button onclick="closeShiftSimModal()" style="background:none;border:none;font-size:22px;cursor:pointer;color:#999;padding:0 4px;line-height:1">&times;</button>' +
      '</div>' +
      '<div id="shiftSimModalBody" style="flex:1;overflow-y:auto;padding:20px"></div>' +
    '</div>';
  document.body.appendChild(overlay);
  return overlay;
}

// opts.admin = true → 管理端「模拟交班」：展示完整金额（含应缴 / 长短款）
// 默认（员工端 · 电子秤终端 personal-shift）→ 防损：不展示销售额与应缴，只记录「备用金 + 实缴」，
// 避免收银员据此推算应缴金额后截留差额。应缴 / 长短款仍在后台计算并入库，仅管理端可见。
function openShiftSimModal(staffOverride, opts) {
  var isAdmin = !!(opts && opts.admin);
  var overlay = simEnsureModal();
  ...
  body.innerHTML =
    ...
    '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px">' +
      ...
      '</div>' +
      '<div style="background:#f8fafc;border-radius:4px;padding:12px 16px">' +
        '<div style="font-size:11px;color:#999;margin-bottom:4px;font-weight:500">订单笔数</div>' +
        '<div id="simOrderCount" style="font-size:15px;font-weight:700;color:#333;font-family:\'SF Mono\',Menlo,monospace">' + (data.orders ? data.orders.length : 0) + ' 笔</div>' +
      '</div>' +
    '</div>' +

    // ===== 在岗期间收款（仅管理端展示；员工端隐藏以防腐损） =====
    (isAdmin ? (
    '<div style="margin-bottom:20px">' +
      '<div style="font-size:12px;font-weight:700;color:#333;margin-bottom:12px;display:flex;align-items:center;gap:6px"><span style="display:inline-block;width:3px;height:14px;background:#005CF5;border-radius:2px"></span>在岗期间收款</div>' +
    ...
    '<div style="margin-bottom:20px">' +
      ...
      (isAdmin ? '' :
        '<div id="simTip" style="margin-bottom:10px;padding:8px 12px;background:#fff8e1;border:1px solid #ffe0b2;border-radius:4px;font-size:11px;color:#e65100;line-height:1.6">本单只记录<b>当班营业收入</b>的上交现金；底钱（备用金）由门店自行管理、不进系统，勿计入下方金额。</div>') +
      '<div style="background:#fff;border:1px solid #e8e8e8;border-radius:6px;padding:0">' +
        // 第一行：结果展示 —— 现金收款 | 应缴现金 | 长短款（仅管理端）
        (isAdmin ? (
        '<div style="display:flex;align-items:stretch;gap:0;padding:0;border-bottom:1px dashed #e8e8e8;flex-wrap:wrap">' +
          ...
          '<div style="flex:1;min-width:90px;padding:14px 12px;text-align:center;border-left:1px solid #f0f0f0">' +
            ...
            '<div id="simDueCash" style="font-size:16px;font-weight:800;color:#ff8f00">¥' + Math.max(data.cashAmt, 0).toFixed(2) + '</div>' +
          '</div>' +
          // 长短款
          ...
          '</div>' +
        '</div>' ) : '') +
        // 输入区：只记录本次上交现金；底钱（备用金）由门店线下自行管理，不进系统
        '<div style="display:flex;align-items:stretch;gap:0;padding:0;flex-wrap:wrap">' +
          '<div style="flex:1;min-width:200px;padding:18px 12px;text-align:center">' +
            '<div style="font-size:10px;color:#666;margin-bottom:6px;font-weight:500">本次上交现金（元）</div>' +
            '<input type="number" id="simActualCash" style="width:100%;max-width:240px;text-align:center;padding:10px 8px;border:2px solid #005CF5;border-radius:4px;font-size:18px;font-weight:800;font-family:\'SF Mono\',Menlo,monospace;color:#005CF5;outline:none;background:#f8fbff;box-sizing:border-box" min="0" step="0.01" oninput="simCalc()" placeholder="仅填当班营业收入" inputmode="decimal">' +
            '<div id="simActualHint" style="font-size:10px;color:#bcaaa4;margin-top:5px">当班营业款（营业收入）</div>' +
            // 口径说明：限定本栏只统计营业收入，杜绝把底钱 / 押金 / 报销 / 私人款项混入
            '<div style="margin:8px auto 0;max-width:300px;padding:7px 10px;background:#fff8e1;border:1px solid #ffe0b2;border-radius:4px;font-size:10px;color:#e65100;line-height:1.6;text-align:left">' +
              '<div style="font-weight:700;margin-bottom:2px">本栏只填当班营业收入（现金部分）</div>' +
              '<div style="color:#a05a00">不计入：底钱（备用金）、押金、报销、预收、私人款项及其他非营业收支</div>' +
            '</div>' +
          '</div>' +
        '</div>' +
      '</div>' +
    ...
    '<div style="margin-bottom:16px">' +
      ...
      '</div>' +
      '<textarea id="simRemark" style="width:100%;padding:10px 12px;border:1px solid #e0e0e0;border-radius:4px;font-size:12px;background:#fff;outline:none;resize:vertical;min-height:56px" placeholder="可点击上方标签快速填入；若上交金额中含非营业款项，请在此说明"></textarea>' +
    '</div>' +

    // ===== 操作按钮 =====
    '<div style="display:flex;gap:10px;justify-content:flex-end;margin-bottom:20px">' +
      '<button class="ic-btn" style="background:#3949ab;color:#fff;border-color:#3949ab;font-weight:600;font-size:14px;padding:0 28px" onclick="simSubmit()">确认交班</button>' +
    '</div>';
  ...
  var titleEl = document.getElementById('shiftSimTitle');
  if (titleEl) titleEl.textContent = isAdmin ? '交班模拟录入' : '交班录入';
...
function simCalc() {
  ...
  var actualEl = document.getElementById('simActualCash');
  var actualCash = actualEl ? (parseFloat(actualEl.value) || 0) : 0;

  // 底钱（备用金）由门店线下自行管理，不进系统 → 应缴 = 当班现金收款
  var dueCash   = Math.max(data.cashAmt, 0);
  var overShort = actualCash - dueCash;

  data.dueCash   = dueCash;
  data.overShort = overShort;

  // 以下两个元素仅管理端（模拟交班）渲染；员工端不存在，天然不展示应缴与长短款
  var dueEl = document.getElementById('simDueCash');
  if (dueEl) dueEl.textContent = '¥' + dueCash.toFixed(2);
  ...
  if (osEl && osBox) {
    if (actualCash === 0) {
      ...
      osBox.style.background = '#fafafa'; osBox.style.borderLeftColor = '#f0f0f0';
    } else {
...
function simSubmit(type) {
  ...
  // 暂不区分「换班 / 下班（闭店）」，统一记为交班；底钱由门店线下管理，不进系统
  type = type || '交班';
...
function simReset() {
  ...
  if (body) {
    ...
    // 应缴 / 长短款仅管理端存在；底钱不进系统，应缴 = 当班现金收款
    el = document.getElementById('simDueCash');     if (el) el.textContent = '¥' + Math.max(data.cashAmt, 0).toFixed(2);
    ...
    // 更新订单数量
    el = document.getElementById('simOrderCount'); if (el) el.textContent = (data.orders ? data.orders.length : 0) + ' 笔';
  }
}
...
function closeShiftSimModal() {
  ...
  // 关闭弹框后刷新交班记录页：按当前页面分别刷新，避免跨页面操作不存在的 DOM
  if (CURRENT_PAGE === 'personal-shift') {
    if (typeof pshRenderTable === 'function') pshRenderTable();
    if (typeof pshRenderCards === 'function') pshRenderCards();
  } else if (CURRENT_PAGE === 'shift-handover') {
...
// 商品列表「活动价」单元格：调用 saCalcPrice 仅展示活动/一口价来源（会员价由独立列展示，不在此重复）
function glSaleActivityCell(r) {
  if (typeof saCalcPrice !== 'function') return '<span style="color:#c0c4cc;font-size:12px">-</span>';
  var res = saCalcPrice(r.goodsId);
  // 纯会员价（无活动）由「会员价」独立列展示；活动价列只展示活动/一口价来源
  if (!res || res.source === 'none' || res.source === 'member') return '<span style="color:#c0c4cc;font-size:12px">-</span>';
  var color = '#f56c6c';
  var tag = (res.detail && res.detail.indexOf('一口价') === 0) ? '一口价' : '活动';
  var strike = res.price < res.original ? ' <span style="color:#c0c4cc;font-size:11px;text-decoration:line-through">¥' + res.original.toFixed(2) + '</span>' : '';
  return '<span style="color:' + color + ';font-weight:600">¥' + res.price.toFixed(2) + '</span>' +
    '<span style="color:#c0c4cc;font-size:11px;margin-left:3px">' + tag + '</span>' + strike;
}

// 商品列表主「售价」列（价格标签）：联动打折活动折后价
// 活动/一口价生效时，主价格列直接显示折后价并划线原价；无活动时保持原价（会员价仍由会员价列单独展示）
function glPriceTagCell(r) {
  if (typeof saCalcPrice !== 'function') return '¥' + r.goodsPrice.toFixed(2);
  var res = saCalcPrice(r.goodsId);
  var isAct = res && (res.source === 'activity' || res.source === 'both-activity');
  if (isAct && res.price < res.original) {
    return '<span style="color:#f56c6c;font-weight:600">¥' + res.price.toFixed(2) + '</span>' +
      '<span style="color:#c0c4cc;font-size:11px;text-decoration:line-through;margin-left:4px">¥' + res.original.toFixed(2) + '</span>';
  }
  return '¥' + r.goodsPrice.toFixed(2);
}
...
function glRenderTable() {
  ...
  tbody.innerHTML = pageData.map(function(r, i) {
    ...
    var expiryTag = (r.expiryManaged === true)
      ? '<span class="gl-tag gl-tag-success">开启</span>'
      : '<span class="gl-tag" style="background:#f4f4f5;color:#909399;border:1px solid #e9e9eb">关闭</span>';
    return '<tr' + selectedClass + ' onclick="glSelectRow(\'' + r.goodsId + '\', this)">' +
      ...
      '<td>' + statusTag + '</td>' +
      '<td>' + expiryTag + '</td>' +
      '<td>' + (r.supplierName || '<span style="color:#c0c4cc;font-size:12px">-</span>') + '</td>' +
      '<td>' + enableTag + '</td>' +
      '<td>' + (r.isSellable === 0 ? '<span style="color:#D46B08;font-size:12px">是</span>' : '') + '</td>' +
      '<td>' + r.shopName + '</td>' +
      '<td>' + glSaleActivityCell(r) + '</td>' +
      '</tr>';
  }).join('');

  if (pageData.length === 0) {
    tbody.innerHTML = '<tr><td colspan="18" style="text-align:center;padding:40px;color:#999">暂无数据</td></tr>';
  }
...
function glOpenAddModal(data) {
  ...
  // 标签 / 供应商：表单可编辑；存量无该字段按空处理
  var goodsTag = data ? (data.goodsTag || '') : '';
  var goodsSupplier = data ? (data.supplierName || '') : '';
  // 效期管理开关：默认关闭（库存不区分效期）；开启后按效期管理库存
  var isExpiryOn = data ? (data.expiryManaged === true) : false;
  ...
  overlay.innerHTML =
    '<div class="gl-modal">' +
      ...
      '<div class="gl-modal-body">' +
        ...
        '</div>' +
        // Row 2: 销售品名(必填,50%) + 商品分类(50%)
        '<div class="gl-form-grid">' +
          '<div class="gl-form-item">' +
            '<span class="gl-form-label"><span class="required">*</span>销售品名</span>' +
            '<div class="gl-form-control">' + glInputWrap('glAddSalesName', salesName, '请输入销售品名') + '</div>' +
          '</div>' +
          '<div class="gl-form-item">' +
        ...
        '<div class="gl-form-grid">' +
          ...
          '</div>' +
          '<div class="gl-form-item">' +
            '<span class="gl-form-label">标签</span>' +
            '<div class="gl-form-control">' + glInputWrap('glAddTag', goodsTag, '如 热销 / 新品 / 促销') + '</div>' +
          '</div>' +
        '</div>' +
        // Row 5: 供应商(100%) — 独占一行
        '<div class="gl-form-grid">' +
          '<div class="gl-form-item">' +
            '<span class="gl-form-label">供应商</span>' +
            '<div class="gl-form-control">' + glBuildSupplierSelect(goodsSupplier) + '</div>' +
          '</div>' +
          '<div style="visibility:hidden"></div>' +
        '</div>' +
        // Row 6: 门店(必填,50%) + 效期管理(50%) — 最底同排
        '<div class="gl-form-grid">' +
          '<div class="gl-form-item">' +
          ...
          '</div>' +
          '<div class="gl-form-item">' +
            '<span class="gl-form-label">效期管理</span>' +
            '<div class="gl-form-control">' +
              '<label style="display:inline-flex;align-items:center;gap:10px;cursor:pointer">' +
                '<span id="glAddExpirySwitch" data-on="' + (isExpiryOn ? '1' : '0') + '" onclick="glToggleExpiry()" style="position:relative;width:42px;height:22px;border-radius:11px;background:' + (isExpiryOn ? '#005CF5' : '#c0c4cc') + ';transition:background .2s;flex-shrink:0">' +
                  '<span style="position:absolute;top:2px;' + (isExpiryOn ? 'left:22px' : 'left:2px') + ';width:18px;height:18px;background:#fff;border-radius:50%;box-shadow:0 1px 3px rgba(0,0,0,.2);transition:left .2s"></span>' +
                '</span>' +
                '<span id="glAddExpiryText" style="font-size:12px;color:#5b6472">' + (isExpiryOn ? '开启 · 按效期管理库存' : '关闭 · 库存不区分效期') + '</span>' +
              '</label>' +
            '</div>' +
          '</div>' +
        '</div>' +
...
function glToggleExpiry() {
  var el = document.getElementById('glAddExpirySwitch');
  if (!el) return;
  var on = el.getAttribute('data-on') !== '0';
  on = !on;
  el.setAttribute('data-on', on ? '1' : '0');
  el.style.background = on ? '#005CF5' : '#c0c4cc';
  var knob = el.querySelector('span');
  if (knob) knob.style.left = on ? '22px' : '2px';
  var txt = document.getElementById('glAddExpiryText');
  if (txt) txt.textContent = on ? '开启 · 按效期管理库存' : '关闭 · 库存不区分效期';
}

function glBuildSupplierSelect(val, id) {
  var list = ['青浦食品厂', '华东农产品批发', '鲜肉专供公司', '光明乳业', '中粮集团', '李锦记', '统一食品'];
  var opts = '<option value="">请选择供应商</option>' + list.map(function (s) {
    return '<option value="' + s + '"' + (s === val ? ' selected' : '') + '>' + s + '</option>';
  }).join('');
  return '<select class="ic-input" id="' + (id || 'glAddSupplier') + '">' + opts + '</select>';
}
function glSaveAddGoods() {
  ...
  var salesName = (document.getElementById('glAddSalesName') || {}).value || '';
  ...
  if (!salesName) { alert('请输入销售品名'); return; }
  ...
  // salesName 已在上方读取并校验
  var goodsTagVal = (document.getElementById('glAddTag') || {}).value || '';
  var supplierVal = (document.getElementById('glAddSupplier') || {}).value || '';

  var expiryEl = document.getElementById('glAddExpirySwitch');
  var expiryManaged = expiryEl ? (expiryEl.getAttribute('data-on') !== '0') : false;

  var sellablePrice = parseFloat(price) || 0;
  var listPriceNum = (GL_ADD_MODE === 'edit' && GL_EDIT_DATA) ? (GL_EDIT_DATA.goodsListPrice || 0) : 0;
  ...
  if (GL_ADD_MODE === 'edit' && GL_EDIT_DATA) {
    ...
    GL_EDIT_DATA.expiryManaged = expiryManaged;
    GL_EDIT_DATA.goodsTag = goodsTagVal;
    GL_EDIT_DATA.supplierName = supplierVal;
...
function glOpenStandardModal(data) {
  ...
  var stdTag = data ? (data.goodsTag || '') : '';
  var stdSupplier = data ? (data.supplierName || '') : '';
  var picUrl = data ? (data.picUrl || '') : '';
  ...
  var stdExpiryManaged = data ? (data.expiryManaged === true) : false;
  // 从规格中解析保质期信息（兼容旧数据）
    ...
    '<div class="gl-modal gl-modal-wide">' +
      ...
      '<div class="gl-modal-body">' +
        ...
        '<div class="gl-form-grid-4">' +
          ...
          '</div>' +
          '<div class="gl-form-item gl-span2">' +
            '<span class="gl-form-label gl-form-label-s">系统品名</span>' +
            '<div class="gl-form-control">' + glInputWrap('glStdLabelName', stdLabelName, '系统品名', 'readonly') + '</div>' +
          '</div>' +
        '</div>' +
        ...
        '<div class="gl-form-grid-4">' +
          ...
          '</div>' +
          '<div class="gl-form-item">' +
            '<span class="gl-form-label gl-form-label-s">标签</span>' +
            '<div class="gl-form-control">' + glInputWrap('glStdTag', stdTag, '如 热销 / 新品 / 促销') + '</div>' +
          '</div>' +
          '<div class="gl-form-item">' +
            '<span class="gl-form-label gl-form-label-s">供应商</span>' +
            '<div class="gl-form-control">' + glBuildSupplierSelect(stdSupplier, 'glStdSupplier') + '</div>' +
          '</div>' +
        '</div>' +
        // Row 5: 门店(单列,25%) + 效期管理(span3,75%)
        '<div class="gl-form-grid-4">' +
          '<div class="gl-form-item">' +
            '<span class="gl-form-label gl-form-label-s"><span class="required">*</span>门店</span>' +
            '<div class="gl-form-control">' + glBuildGenericSelect('glStdShop', stdShop, '请选择门店', 'shops') + '</div>' +
          '</div>' +
          '<div class="gl-form-item gl-span3">' +
            '<span class="gl-form-label gl-form-label-s">效期管理</span>' +
            '<div class="gl-form-control">' +
              '<label style="display:inline-flex;align-items:center;gap:10px;cursor:pointer">' +
                '<span id="glStdExpirySwitch" data-on="' + (stdExpiryManaged ? '1' : '0') + '" onclick="glStdToggleExpiry()" style="position:relative;width:42px;height:22px;border-radius:11px;background:' + (stdExpiryManaged ? '#005CF5' : '#c0c4cc') + ';transition:background .2s;flex-shrink:0">' +
                  '<span style="position:absolute;top:2px;' + (stdExpiryManaged ? 'left:22px' : 'left:2px') + ';width:18px;height:18px;background:#fff;border-radius:50%;box-shadow:0 1px 3px rgba(0,0,0,.2);transition:left .2s"></span>' +
                '</span>' +
                '<span id="glStdExpiryText" style="font-size:12px;color:#5b6472">' + (stdExpiryManaged ? '开启 · 按效期管理库存' : '关闭 · 库存不区分效期') + '</span>' +
              '</label>' +
            '</div>' +
          '</div>' +
        '</div>' +
...
function glStdToggleExpiry() {
  var el = document.getElementById('glStdExpirySwitch');
  if (!el) return;
  ...
  var knob = el.querySelector('span');
  if (knob) knob.style.left = on ? '22px' : '2px';
  var txt = document.getElementById('glStdExpiryText');
  if (txt) txt.textContent = on ? '开启 · 按效期管理库存' : '关闭 · 库存不区分效期';
}
...
function glSaveStandard() {
  ...
  var stdExpiryEl = document.getElementById('glStdExpirySwitch');
  var expiryManaged = stdExpiryEl ? (stdExpiryEl.getAttribute('data-on') === '1') : false;
  ...
  var tag = document.getElementById('glStdTag').value.trim();
  var supplier = document.getElementById('glStdSupplier').value;
  ...
  if (GL_STD_ADD_MODE === 'edit' && GL_STD_EDIT_DATA) {
    ...
    GL_STD_EDIT_DATA.goodsTag = tag;
    GL_STD_EDIT_DATA.supplierName = supplier;
    if (picUrl) GL_STD_EDIT_DATA.picUrl = picUrl;
    GL_STD_EDIT_DATA.expiryManaged = expiryManaged;
  } else {
